...
function StatusIcon({ icon }: { readonly icon: ActivityStatusIcon }) {
  if (icon.kind === 'dot') {
    return <span className="yapi-status-icon-dot" aria-hidden="true" />;
  }
  if (icon.kind === 'working') {
    return (
      <span className="yapi-working-icon" aria-hidden="true">
        <svg viewBox="0 0 16 16" width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <circle className="yapi-working-icon-track" cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.5" opacity="0.25" />
          <path className="yapi-working-icon-arc" d="M8 2a6 6 0 0 1 6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </span>
    );
  }
...
export function ActivityStatusBadge({ status }: { readonly status: ActivityStatus }) {
  ...
  return (
    <span
      ...
      aria-atomic="true"
      aria-live="polite"
      className={`yapi-activity-status yapi-tool-status status-${status}`}
    >
      <StatusIcon icon={presentation.icon} />
      <span className="yapi-activity-status-label">{presentation.label}</span>
    </span>
  );
}
...
export function ActivityStatusCountSummary({ statuses }: { readonly statuses: readonly ActivityStatus[] }) {
  ...
  return (
    <span
      ...
      aria-atomic="true"
      aria-live="polite"
      className="yapi-tool-step-group-status"
    >
      {items.map((item, index) => (
        <Fragment key={item.status}>
          {index > 0 ? <span aria-hidden="true" className="yapi-tool-step-group-status-separator">/</span> : null}
          <span aria-hidden="true" className={`yapi-activity-status yapi-tool-status status-${item.status}`}>
            <StatusIcon icon={item.icon} />
            <span className="yapi-activity-status-label">{item.countLabel}</span>
          </span>
        </Fragment>
      ))}
    </span>
...
function ActivityElapsed({ completedAt, startedAt, status }: {
  ...
  return (
    <time
      aria-hidden="true"
      className="yapi-activity-elapsed"
...
export function ActivityRow({ completedAt, icon, meta, name, startedAt, status, summary }: ActivityRowProps) {
  return (
    <>
      <span className="yapi-activity-icon yapi-tool-icon" aria-hidden="true">{icon}</span>
      <span className="yapi-activity-name yapi-tool-name">{name}</span>
      <span className="yapi-activity-summary yapi-tool-summary">{summary}</span>
      {meta}
