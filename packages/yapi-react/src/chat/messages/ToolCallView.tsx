...
function GenericToolContent({ toolCall }: { readonly toolCall: ToolCallInfo }) {
  ...
  if (toolCall.status === 'running' && !toolCall.result) {
    return <div className="yapi-tool-result-row"><span className="yapi-tool-result-text">{t('chat.stream.writing')}</span></div>;
  }
  if (!toolCall.result) return null;
  return <pre className="yapi-tool-lines yapi-tool-raw-text">{toolCall.result}</pre>;
}
  ...
  if (adapter) {
    return (
      <ImperativeContentSlot
        adapter={adapter}
        className="yapi-tool-content-adapter"
...
function ToolCallPresentation({ toolCall, contentAdapters, compact = false }: {
  ...
  if (toolCall.subagent && contentAdapters?.subagent) {
    return (
      <ImperativeContentSlot
        ...
        className="yapi-subagent-content-adapter"
  ...
  const diffStats = isWriteEditTool(toolCall.name)
    ...
    ? (
      <span className="yapi-write-edit-stats">
  ...
  const className = [
    'yapi-tool-call',
    descriptor.className ? `yapi-tool-call-${descriptor.className}` : '',
    compact ? 'yapi-tool-call-in-step-group yapi-tool-call-compact' : '',
    expanded ? 'expanded' : '',
  ...
  return (
    <div className={className} data-tool-id={toolCall.id}>
      ...
      >
        ...
        />
        <span aria-hidden="true" className={`yapi-collapsible-chevron${expanded ? '' : ' is-collapsed'}`}>
          <PlatformIcon name="chevron-down" />
        </span>
      </button>
      {expanded && <div className="yapi-tool-content"><ToolContent toolCall={toolCall} contentAdapters={contentAdapters} /></div>}
    </div>
  );
}
...
function ToolStepGroupPresentation({ toolCalls, contentAdapters, projectionStore }: {
  ...
  return (
    <div className={`yapi-tool-step-group${expanded ? ' expanded' : ''}${status === 'running' ? ' is-running' : ''}`}>
      <button
        type="button"
        className="yapi-tool-step-group-header"
        aria-expanded={expanded}
      ...
      >
        <span className="yapi-tool-step-group-count">{countLabel}</span>
        <span className="yapi-tool-step-group-summary" aria-hidden="true">{toolNamesLabel}</span>
      ...
      {expanded && (
        <div className="yapi-tool-step-group-steps">
          {toolCalls.map(toolCall => (
            <div className="yapi-tool-step-item" key={toolCall.id}>
              {projectionStore
