import type { ChatMessage, ContentBlock, SubagentInfo, ToolCallInfo } from '@yapi/yapi-agent-core/foundation';
  ...
  if (contentAdapters?.markdown) {
    return (
      <MessageContentSlot
        ...
        className="yapi-text-block"
      />
    );
  }
  return <div className="yapi-text-block">{block.content}</div>;
}
...
function ThinkingBlockView({
  ...
  return (
    <details aria-label={t('chat.stream.thinkingExpandAria')} className="yapi-thinking-block">
      <summary className="yapi-thinking-header">
        <span className="yapi-thinking-label">
          {seconds === null ? t('chat.stream.thought') : t('chat.stream.thoughtFor', { seconds })}
        </span>
      </summary>
      {contentAdapters?.markdown
        ? <MessageContentSlot adapter={contentAdapters.markdown} className="yapi-thinking-content" generation={generation} value={{ blockId: generation, content: block.content, phase: isStreaming ? 'streaming' : 'terminal' }} />
        : <div className="yapi-thinking-content">{block.content}</div>}
    </details>
  );
}
...
export function AssistantContentView({ message, contentAdapters, isStreaming = false, projectionStore }: AssistantContentViewProps) {
  ...
  if (message.durationSeconds && message.durationSeconds > 0 && !hasCompactBoundary) {
    content.push(
      <div className="yapi-response-footer" key={`${message.id}:duration`}>
        <span className="yapi-baked-duration yapi-response-meta">
