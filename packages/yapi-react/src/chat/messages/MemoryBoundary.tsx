import type { CheckpointPresentation } from '@yapi/yapi-agent-core/foundation';
...
export function MemoryBoundary({
  ...
  const chipContent = (
    <>
      <span className="yapi-memory-chip-label">{label}</span>
      {before && after ? (
        <span aria-hidden="true" className="yapi-memory-chip-transition">{before} → {after}</span>
      ) : null}
    </>
  );

  return (
    <div className="yapi-memory-root">
      <div
        aria-label={accessibleLabel}
        className={`yapi-memory-boundary${kind === 'compaction' ? ' yapi-compact-boundary' : ' yapi-history-boundary'}`}
        role="separator"
      />
      {hasDetails ? (
        <button
          ...
          className="yapi-memory-chip yapi-memory-chip-button"
          onClick={() => setIsExpanded((value) => !value)}
          type="button"
        >
          {chipContent}
        </button>
      ) : <span className="yapi-memory-chip">{chipContent}</span>}
      {hasDetails && isExpanded ? (
        <section
          aria-label={t('chat.stream.checkpointDetails')}
          className="yapi-checkpoint-panel"
          id={panelId}
        >
          <p className="yapi-checkpoint-summary">
...
function CheckpointDetails({ checkpoint }: { checkpoint: CheckpointPresentation }) {
  const t = useT();
  return (
    <div className="yapi-checkpoint-details">
      {checkpoint.goal ? (
      ...
      <dl className="yapi-checkpoint-meta">
        <div>
...
function CheckpointSection({ label, values }: { label: string; values: string[] }) {
  ...
  return (
    <section className="yapi-checkpoint-section">
      <h4>{label}</h4>
