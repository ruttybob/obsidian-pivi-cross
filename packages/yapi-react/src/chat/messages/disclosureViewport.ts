...
const DISCLOSURE_HEIGHT_PROPERTY = '--yapi-expanded-content-max-height';
const SUBAGENT_HEIGHT_PROPERTY = '--yapi-subagent-expanded-max-height';
...
const STEP_GROUP_STICKY_TOP_PROPERTY = '--yapi-tool-step-group-sticky-top';
...
function getDisclosureRow(header: HTMLElement): HTMLElement | null {
  return header.closest<HTMLElement>('.yapi-message-virtual-row');
}
...
export function createDisclosureViewportController(
  ...
  const syncStepGroupStickyOffsets = () => {
    for (const group of scrollElement.querySelectorAll<HTMLElement>(
      '.yapi-tool-step-group.expanded',
    )) {
      const header = group.querySelector<HTMLElement>(
        ':scope > .yapi-tool-step-group-header',
      );
      if (!header) continue;
