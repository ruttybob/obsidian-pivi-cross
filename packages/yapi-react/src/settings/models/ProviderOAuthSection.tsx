...
export function ProviderOAuthSection({
  ...
  return (
    <div className="yapi-provider-oauth-setting yapi-setting-stack">
      <SettingRow name={t(keys.name)} description={t(keys.desc, { secureStorageName })}>
        <div className="yapi-provider-oauth-toolbar">
          <div className="yapi-provider-oauth-actions">
          ...
          {deviceCode ? (
            <div className="yapi-provider-oauth-code" aria-live="polite">
              <span className="yapi-provider-oauth-code__label">{t(OAUTH_COMMON_SETTINGS_KEYS.deviceCode)}</span>
              <code className="yapi-provider-oauth-code__value">{deviceCode}</code>
            </div>
          ) : null}
        </div>
      </SettingRow>
