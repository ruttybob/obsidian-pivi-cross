...
export function ProviderApiKeyField({
  ...
  return (
    <div className="yapi-cred-row yapi-setting-stack">
      <SettingRow
...
export function ProviderCredentials({ models, providerId, allowKeyless, onChanged, onError }: ProviderCredentialsProps) {
  ...
  return (
    <>
      ...
      {showOauthToggle ? (
        <div className="yapi-auth-toggle-wrapper">
          <button
            className={`yapi-auth-toggle-btn${authType === 'api' ? ' active' : ''}`}
            type="button"
          ...
          <button
            className={`yapi-auth-toggle-btn${authType === 'oauth' ? ' active' : ''}`}
            type="button"
