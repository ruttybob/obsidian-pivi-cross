...
function EnvironmentSection({ environment, feedback }: {
  ...
  return (
    <SettingsSection title={t('settings.environment')}>
      {reviewKeys.length > 0 ? (
        <div className="yapi-env-review-warning yapi-setting-validation yapi-setting-validation-warning">
      ...
      <SettingRow name={t('settings.sharedEnvironment.name')} description={t('settings.sharedEnvironment.desc')}>
        <textarea
          className="yapi-settings-control yapi-settings-control--fill yapi-settings-env-textarea"
          rows={6}
...
function HotkeyGrid({ hotkeys }: { readonly hotkeys: SettingsHotkeysPort }) {
  ...
  return (
    <div className="yapi-hotkey-grid">
      {rows.map((row) => (
        <button
          key={row.commandId}
          type="button"
          className="yapi-hotkey-item"
          onClick={() => hotkeys.openHotkeySettings()}
        >
          <span className="yapi-hotkey-name">{t(row.labelKey as TranslationKey)}</span>
          {row.hotkey ? <span className="yapi-hotkey-badge">{row.hotkey}</span> : null}
        </button>
      ))}
    </div>
  );
  ...
  return (
    <>
      <SettingRow name={t('settings.navMappings.name')} description={t('settings.navMappings.desc')}>
        <div className="yapi-settings-control-feedback">
          <textarea
    ...
    <>
      ...
      <SettingsSection title={t('settings.compaction.title')}>
        ...
        <SettingRow name={t('settings.compaction.threshold.name')} description={t('settings.compaction.threshold.desc')}>
          <div className="yapi-settings-range">
            <input
              type="range"
              min="50"
              max="95"
            ...
            />
            <output className="yapi-settings-range__value">
      ...
      <SettingsSection title={t('settings.personalizationContext')}>
        ...
        <div className="yapi-setting-stack">
  ...
  return (
    <SettingsSection title={t('settings.sessionFiles.heading')}>
      <SettingRow name={t('settings.sessionFiles.deleteRemoved.name')} description={t('settings.sessionFiles.deleteRemoved.desc')}>
        <button className="yapi-button--danger" type="button" disabled={pending} onClick={() => { void clean(); }}>
  ...
  return (
    <SettingsSection title={t('settings.integrations.heading')}>
      ...
      {sections.map((section) => (
        <div key={section.id} className="yapi-integration-setting yapi-setting-stack">
          <SettingRow name={section.heading} description={section.description}>
            {section.actions.map((action) => {
              const disabledFeedback = action.disabledReason
                ...
                : null;
              return (
                <span className="yapi-settings-action-group" key={action.id}>
                  <button
                    type="button"
