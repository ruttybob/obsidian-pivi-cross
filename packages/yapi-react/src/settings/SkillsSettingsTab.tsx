  ...
  return (
    <>
      <SettingsPageDescription>
        <p className="yapi-setting-description">{t('settings.skills.intro')}</p>
      ...
      <div className="yapi-skills-remote-setting yapi-setting-stack">
        <SettingRow name={t('settings.skills.remote.name')} description={t('settings.skills.remote.desc')}>
          <input
            className="yapi-settings-control"
            value={source}
            onChange={(event) => {
      ...
      {remote.length > 0 ? (
        <div className="yapi-skills-remote-host">
          <SettingsListHeader
            title={t('settings.skills.remote.heading')}
            actions={(
              <button
                type="button"
                className="yapi-settings-text-btn"
                disabled={busy}
          ...
          />
          <div className="yapi-sp-list yapi-skills-remote-list">
            {remote.map((skill) => (
              <label className="yapi-skill-choice" key={skill.name}>
                <input
                  type="checkbox"
                  className="yapi-skill-choice-checkbox"
                ...
                />
                <span className="yapi-skill-choice-info">
                  <span className="yapi-sp-item-name">{skill.name}</span>
                  {skill.description ? <span className="yapi-sp-item-desc">{skill.description}</span> : null}
                </span>
              </label>
            ))}
          </div>
          <div className="yapi-skills-install-actions">
            <button
              type="button"
              className="yapi-button--primary yapi-skills-install-selected-btn"
              disabled={busy}
      ...
      <SettingsListHeader
        ...
        actions={(
          <button
            type="button"
            className="yapi-settings-action-btn"
            disabled={busy}
      ...
      />
      {entries.length === 0 ? (
        <p className="yapi-sp-empty-state">{t('settings.skills.installed.empty')}</p>
      ) : (
        <div className="yapi-sp-list">
          {entries.map((skill) => (
            <div className="yapi-sp-item" key={skill.folderName}>
              <div className="yapi-sp-info">
                <div className="yapi-sp-item-header">
                  <span className="yapi-sp-item-name">{skill.name}</span>
                  <span className="yapi-sp-item-folder">{skill.folderName}</span>
                  {skill.disabled ? <span className="yapi-slash-item-badge">{t('common.disabled')}</span> : null}
                </div>
                {skill.description ? <div className="yapi-sp-item-desc">{skill.description}</div> : null}
              </div>
              <div className="yapi-sp-item-actions">
                <button
                  type="button"
                  className="yapi-settings-text-btn"
                  disabled={busy}
                ...
                <button
                  type="button"
                  className="yapi-settings-action-btn yapi-settings-delete-btn"
                  disabled={busy}
