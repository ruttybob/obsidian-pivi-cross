...
export function ChatTabBar({ shell, ownerWindow }: { shell: ChatShellOptions; ownerWindow: Window }) {
  ...
  useEffect(() => {
    ...
    const menuItems = Array.from(
      containerRef.current?.querySelectorAll<HTMLElement>('.yapi-tab-switcher-item') ?? [],
    );
  ...
  const renderItem = (item: ChatTabSnapshotItem) => {
    ...
    return (
      <div
        ...
        className={`yapi-tab-switcher-item${item.isActive ? ' is-active' : ''}${item.needsAttention ? ' needs-attention' : ''}${item.isArchived ? ' is-archived' : ''}${exiting ? ' is-exiting' : ''}${editing ? ' is-editing' : ''}`}
        data-tab-id={item.id}
        key={item.id}
        onClick={(event) => {
          event.stopPropagation();
        ...
        tabIndex={0}
      >
        <span className={`yapi-tab-switcher-dot${dotClass(item)}`} />
        <span className="yapi-tab-switcher-item-title">
          {editing
            ? (
              <EditableTabTitle
                item={item}
        ...
        <TabAction
          className="yapi-tab-switcher-action yapi-tab-switcher-edit-title"
          icon="pencil"
        ...
        />
        <TabAction
          className="yapi-tab-switcher-action yapi-tab-switcher-archive"
        ...
        {item.canClose
          ? (
            <TabAction
              className="yapi-tab-switcher-action yapi-tab-switcher-close"
              icon="x"
  ...
  return (
    <div className={`yapi-tab-switcher${isOpen ? ' is-open' : ''}`} ref={containerRef}>
      {isOpen || isClosing
        ? (
          <div
            className={`yapi-tab-switcher-menu${isArchivedRevealed ? ' is-archived-revealed' : ''}${isClosing ? ' is-closing' : ''}`}
          ...
          >
            ...
            {archivedItems.length > 0
              ? <div className="yapi-tab-switcher-section-label">{t('chat.tabs.archived')}</div>
              : null}
            {archivedItems.map(renderItem)}
          </div>
        )
        : null}
      <div className="yapi-tab-switcher-control">
        <TabAction
          className="yapi-tab-switcher-new-chat"
          icon="square-pen"
        ...
        <span
          ...
          className="yapi-tab-switcher-trigger"
          onClick={(event) => {
            event.stopPropagation();
            if (isOpen) closeMenu();
            else openMenu(false);
          ...
          tabIndex={0}
        >
          <span className={`yapi-tab-switcher-dot${dotClass(activeItem)}`} />
          <ActiveTabTitle item={activeItem} ownerWindow={ownerWindow} />
          <span className="yapi-tab-switcher-chevron"><PlatformIcon name="chevron-up" /></span>
        </span>
      </div>
    </div>
  );
