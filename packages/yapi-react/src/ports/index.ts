...
import type { AppModelReadinessStatusKind } from '@yapi/yapi-agent-core/foundation/modelReadiness';
import type {
  EnvironmentScope,
  WebProviderId,
...
export type { ProviderOAuthProgress } from '@yapi/yapi-agent-core/auth/providerOAuthProgress';
...
export interface SettingsPersistencePort {
  getSettingsSnapshot(): YapiSettings;
  commitSettingsSnapshot(snapshot: YapiSettings): Promise<void>;
}
...
export interface SettingsCatalogPort {
  ...
  syncCustomProviders(snapshot: YapiSettings): void;
  fetchCustomProviderModels(
    providerId: string,
    snapshot: YapiSettings,
