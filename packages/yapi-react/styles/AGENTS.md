...
- `packages/yapi-react/styles/` contains the modular CSS source for YaPi's product React UI.
...
- `scripts/build-css.mjs` prepends `packages/obsidian-host/styles/yapi-theme.css`, then resolves the 37 manifest paths within this directory, concatenates modules in declaration order, and writes root `styles.css`.
...
- `settings/`: settings navigation and shared layout plus slash, MCP, and agent-specific rules. `settings/base.css` owns shared `.yapi-sp-*` structures.
...
- Prefix plugin-owned classes, custom properties, animations, and highlight names with `yapi-` (`.yapi-*`, `--yapi-*`, `yapi-*`). Strictly YaPi-scoped integration rules may consume upstream Markdown renderer, CodeMirror, or Lucide variables, but must not expose them as YaPi-owned public tokens.
- Scope overrides beneath a YaPi root such as `.yapi-container`, `.yapi-settings`, or `.yapi-inline-edit-modal` when touching Obsidian or CodeMirror elements.
- Components consume `--yapi-host-*` theme tokens and shared `--yapi-*` product tokens. Each note-host maps its theme system into that contract; the Obsidian mapping lives in `packages/obsidian-host/styles/yapi-theme.css`.
- Shared product spacing, radius, elevation, material, motion, surface, typography, focus, and press tokens are declared together for `.yapi-container`, `.yapi-settings`, and `.yapi-inline-edit-modal`. Keep Style Settings-backed chat and composer typography as separate semantic tokens.
- Product CSS targets only package-owned structural classes. Host adapters may normalize rendered third-party markup onto `.yapi-*` classes before package CSS consumes it; do not target host classes such as `setting-item`, `modal-*`, `svg-icon`, or theme-marker classes here.
- Settings form controls share `.yapi-settings-control` for sizing, borders, focus, placeholder, and disabled states. Use `--fill` only for intentional full-width editors; checkbox, toggle, and range controls are excluded. Badge-list fields own wrapping, long-token overflow, and compact per-badge remove buttons in `settings/base.css`.
- Compact control feedback uses `.yapi-settings-action-feedback` beside its owning action: host green for success and host error red for failure. `.yapi-settings-action-group` and `.yapi-settings-control-feedback` provide shared wrapping/layout; feature styles must not recreate page-level result banners.
- Settings page descriptions, row content, section labels, and list-header titles align to `--yapi-settings-gutter`. `SettingsSection` owns asymmetric section spacing (`--yapi-settings-section-gap` before, `--yapi-settings-section-title-gap` after the label); nested subsections inside a section body use `--yapi-settings-subsection-gap`. Section headings reset host `h2`/`h3` margin/padding under `.yapi-settings`. Lists and cards stay full width with internal padding; feature CSS must not introduce one-off row insets. A list header owns the header-to-list gap, so the following list must not add another top margin.
- Native settings selectors must render through the shared React `Select` control and `.yapi-select` primitive. Keep them content-sized with a 100% container cap and the shared height, border, focus, hover, and disabled states; feature CSS must not stretch a selector to fill a row. Custom composer/menu selectors remain separate interaction components.
...
- Toolbar dropdowns rely on coordinated positioning, hover/focus states, and high `z-index` values; changing overflow or stacking contexts in the composer can hide them. `.yapi-input-container` stays above `.yapi-messages-bottom-controls` so model/thinking/external menus cover the tab switcher.
- `.yapi-input-container` has no extra bottom inset (`padding: 0`); the toolbar stays at the sidebar bottom as the input wrapper's last column-flex portal child. Keep dropdown stacking / focus-within rules so menus cover the tab switcher.
...
- Inline-edit styles target CodeMirror 6 decorations and `.yapi-inline-edit-modal.cm-editor`; broad selectors can leak into normal editors.
