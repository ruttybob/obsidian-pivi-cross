# @yapi/obsidian-host package guide

*This file extends the root [AGENTS.md](../../AGENTS.md). Follow root guidance first, then these package-specific rules.*

## Purpose

`@yapi/obsidian-host` is the Obsidian/Electron host adapter layer. It wraps vault APIs, file stores, settings persistence, shared storage, secret storage, host context, path normalization, and renderer compatibility helpers.
...
- `src/settings/` owns vault settings persistence and the codec contract used to inject product/runtime normalization plus an optional pre-save projection. Product defaults live in `@yapi/yapi-agent-core/foundation`; Pi-specific normalization and device-local-field stripping live outside this package.
...
- `src/providerLegacyAuthStore.ts` adapts the canonical provider legacy auth store port to the vault-local `.yapi/auth.json` file used only for old Codex credential migration.
...
- `styles/yapi-theme.css` maps Obsidian theme variables into the `--yapi-host-*` contract consumed by `@yapi/yapi-react`; the root CSS build prepends it directly before the React style manifest. It is a build input, not a JavaScript package export.
...
- Implement host adapters against `@yapi/yapi-agent-core/ports` (file stores, secrets, HTTP, process, openers). App composition injects those adapters into the Pi engine; the engine must not import this package.
- Do not import UI components or Pi engine implementation details. Expose host capabilities through typed contracts.
- Do not import `@yapi/yapi-agent-core/engine/pi`, `@yapi/yapi-agent-core/skills`, `@yapi/yapi-agent-core/tools`, or concrete Obsidian tool implementations; app composition and tool packages should add product/runtime/tool semantics above the host layer.
