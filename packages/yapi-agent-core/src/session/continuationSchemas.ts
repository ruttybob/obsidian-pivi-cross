...
export interface YapiCompactionDetails {
  yapiCheckpoint: Checkpoint;
}
...
export const AGENT_REPORT_BLOCK_LANGUAGE = 'yapi-agent-report';
...
export function parseYapiCompactionDetails(value: unknown): YapiCompactionDetails | null {
  if (!isRecord(value)) {
    return null;
  }
  const yapiCheckpoint = parseCheckpoint(value.yapiCheckpoint);
  return yapiCheckpoint ? { yapiCheckpoint } : null;
}
