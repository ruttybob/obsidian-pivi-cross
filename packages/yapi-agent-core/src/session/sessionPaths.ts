...
/** Vault-local pi-compatible session root: `<vault>/.yapi/sessions/--<encoded>--/`. */
export function getYapiSessionDir(vaultPath: string): string {
  return path.join(vaultPath, '.yapi', 'sessions', encodeSessionCwd(vaultPath));
}
