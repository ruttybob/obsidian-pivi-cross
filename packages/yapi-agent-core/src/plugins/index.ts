...
export interface YapiPluginResources {
  skills?: string[];
  prompts?: string[];
  tools?: string[];
...
export interface YapiPluginManifest {
  id: string;
  name: string;
  version?: string;
  source: PluginSource;
  resources: YapiPluginResources;
...
export interface PluginLockRecord {
  ...
  resolvedRef?: string;
  integrity?: string;
  enabledResources?: YapiPluginResources;
...
function parseResources(value: unknown): YapiPluginResources {
  if (value === undefined) {
    return {};
  }
  if (!isRecord(value)) {
  ...
  const resources: YapiPluginResources = {};
...
export function parseYapiPluginManifest(input: unknown): YapiPluginManifest {
  if (!isRecord(input)) {
