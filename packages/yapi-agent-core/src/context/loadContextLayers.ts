...
import { YAPI_STORAGE_PATH } from '../skills/vault/paths';

const YAPI_SYSTEM_PROMPT_PATH = `${YAPI_STORAGE_PATH}/SYSTEM.md`;
...
export function loadContextLayers(
  ...
  const systemMd = readFileIfExists(path.join(vaultPath, YAPI_SYSTEM_PROMPT_PATH)).trim();
