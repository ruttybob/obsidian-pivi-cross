...
const INLINE_CONTEXT_TOKEN_PREFIX = '@[yapi-inline-context:';
const INLINE_CONTEXT_TOKEN_REGEX = /@\[yapi-inline-context:([A-Za-z0-9_-]+)\]/g;
...
export function parseInlineContextToken(token: string): InlineContextReference | null {
  const match = token.match(/^@\[yapi-inline-context:([A-Za-z0-9_-]+)\]$/);
