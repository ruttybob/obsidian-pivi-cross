...
`packages/yapi-agent-core/src/engine/pi/` is YaPi's adapter boundary around `@earendil-works/pi-agent-core`, `@earendil-works/pi-ai`, and the Pi coding-agent session implementation. It constructs in-process Pi agents, adapts YaPi tools and sessions to Pi SDK types, configures providers/authentication, and exposes host-neutral implementations of `PiChatService` and `AuxQueryRunner`.

This is the **only application source directory where raw `@earendil-works/*` imports are allowed**. Code outside this directory must consume YaPi-owned contracts and exports.

## Architecture

```mermaid
flowchart TD
  App["App composition<br/>src/app"] -- "injects host ports, settings,<br/>tool provider, MCP" --> Runtime["PiChatRuntime<br/>packages/yapi-agent-core/src/engine/pi/piChatRuntime.ts"]
  UI["Product UI<br/>src/ui"] --> Contract["PiChatService / AuxQueryRunner<br/>packages/yapi-agent-core/src/runtime"]
  ...
  Registry --> BaseTools["Injected Yapi ToolSpec values"]
  ...
  Tree -- "implements persistence behavior for" --> SessionContracts["Yapi session contracts<br/>packages/yapi-agent-core/src/session"]
...
- `packages/yapi-agent-core/src/engine/pi/piChatRuntime.ts` is the concrete `PiChatService`.
...
- `packages/yapi-agent-core/src/engine/pi/piAgentEventAdapter.ts` is the sole Pi-event-to-`StreamChunk` translator. It maps text, thinking, tool lifecycle, completion, and errors; do not leak raw `AgentEvent` values to UI/runtime consumers.
- `packages/yapi-agent-core/src/engine/pi/piRuntimeHost.ts` defines the narrow host surface available to the runtime.
- `packages/yapi-agent-core/src/engine/pi/piImageContent.ts` maps YaPi image attachments into pi-ai image content.
...
- `packages/yapi-agent-core/src/engine/pi/buildPiToolRegistryCore.ts` owns registry composition.
  - The app injects host-specific tools as YaPi `ToolSpec` values through `PiBaseToolProvider`.
...
- `packages/yapi-agent-core/src/engine/pi/createSkillTool.ts` exposes loaded vault skill bodies without moving skill parsing into the Pi layer.
...
- `packages/yapi-agent-core/src/engine/pi/piAuxQueryRunner.ts` constructs lightweight, low-thinking Pi agents for title generation, inline/refine queries, compaction, and subagents.
...
- `packages/yapi-agent-core/src/engine/pi/piModelRegistry.ts` caches models by `provider/modelId`, builds UI options, and tracks whether context-window metadata is authoritative.
- `packages/yapi-agent-core/src/engine/pi/piModelEnv.ts` resolves settings fallback models and delegates credential resolution through YaPi auth ports and `piAiModels`.
...
- `packages/yapi-agent-core/src/engine/pi/piProviderOAuthService.ts` owns interactive provider OAuth for OpenAI Codex, Grok Build (`grok-build`), and Claude (`claude`): pi-ai login/logout uses the exact provider id and therefore touches only that provider's credential. OAuth descriptions explain the required xAI account or Claude Pro/Max subscription without folding the authentication method into the provider name. Legacy Codex auth migration still uses injected storage ports. `piAuthInteraction.ts` maps browser URLs, device-code pages, and manual-code prompts onto the OAuth host; manual-code waits observe both pi-ai's prompt signal and the YaPi login cancellation signal. `yapiXaiOAuthDeviceFlow.ts` uses injected fetch and prefers `verification_uri_complete` over `verification_uri` before opening the browser.
- `packages/yapi-agent-core/src/engine/pi/piThinkingLevels.ts`, `piChatUiConfig.ts`, and `piSettingsCoordinator.ts` project Pi model/reasoning behavior into YaPi-owned UI/settings contracts.

## Session relationship

- `packages/yapi-agent-core/src/session/` owns host-neutral contracts, identity, path helpers, UI metadata types, and session-management interfaces. It must not import Pi SDK types.
- `packages/yapi-agent-core/src/engine/pi/session/` implements those contracts using Pi JSONL and `SessionManager`:
  - `sessionTreeStore.ts` isolates Pi `SessionManager`, append/sync/fork/truncate behavior, compaction entries, and YaPi custom JSONL entries. Import Pi session types from the package root; private `dist/core/*` paths are not a supported boundary.
  - `sessionJsonlIndex.ts` owns the rebuildable `.yapi-index` sidecar: UTF-8 byte offsets, projection metadata, per-line hashes, append-only checkpoints, migration markers, bounded source fingerprints, incremental append refresh, rewrite invalidation, and verified indexed-line reads. JSONL remains authoritative; low-level stale/corrupt validation raises the typed errors owned by `session/types.ts`, while read-only consumers discard that optimization and rebuild before returning data. Held mutation fingerprints never auto-recover.
  - `piSessionStore.ts` implements the package-level `SessionStore` and maps durable JSONL sessions to YaPi `SessionRef`, `ChatMessage`, usage, metadata, and UI context. Identity, history summaries, usage, UI context, and bounded message pages read verified indexed lines; full `getMessages()` remains the explicit runtime/compatibility path. Absolute external-context paths are removed at every JSONL write and overlaid from an injected device-local store on read. Startup/lazy-open migration consults the sidecar marker, single-flights each file, and rewrites only legacy JSONL while preserving line order/final-newline shape; marker-complete files never reread the body. Startup warns and skips a malformed session so one corrupt JSONL cannot block the plugin; explicitly opening that session still fails with its file/line.
  - `agentMessageHistory.ts` compares and sanitizes Pi message history before persistence or LLM submission.
  - `messageMapper.ts` reconstructs visible YaPi messages, tool calls, images, and UI overlays from Pi entries.
...
- YaPi restores an existing session as a **linear append-order conversation**, not as a selected Pi tree leaf. Tree support remains a compatibility detail used for old files and creating a fork in a new session file.
...
- Raw imports matching `@earendil-works/*` belong only under `packages/yapi-agent-core/src/engine/pi/`. Do not spread Pi SDK types into `foundation/`, `runtime/`, `tools/`, `session/`, MCP, host packages, app code, or UI.
- Export YaPi-owned contracts at the boundary: `PiChatService`, `AuxQueryRunner`, `ToolSpec`, `StreamChunk`, `SessionStore`, and related foundation/session types.
- Do not import `obsidian`, `electron`, `@yapi/obsidian-host`, `@yapi/obsidian-tools`, product UI, or app implementation modules here.
...
- `packages/yapi-agent-core/src/engine/pi/shims/piCodingAgentConfig.ts` replaces upstream coding-agent config because its top-level `import.meta.url` is incompatible with YaPi's bundled CommonJS `main.js`. Its version/constants must stay aligned with the installed Pi package.
- `packages/yapi-agent-core/src/engine/pi/shims/piAiCompat.ts` provides the compatibility API expected by upstream code while routing model lookup and streaming through YaPi's shared `piAiModels` registry. It also supports dynamically registered API providers and YaPi's environment-key shim.
- `packages/yapi-agent-core/src/engine/pi/shims/piAiEnvApiKeys.ts` replaces upstream dynamic `import("node:" + "fs")` behavior, which Electron's renderer can treat as a URL fetch. It uses synchronous Node modules behind a configurable environment host.
- `packages/yapi-agent-core/src/engine/pi/shims/signalExit.cjs` supplies the callable CommonJS shape expected by `proper-lockfile`. Exit cleanup is intentionally a no-op inside Obsidian.
