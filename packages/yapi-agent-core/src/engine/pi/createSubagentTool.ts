    ...
    async execute(_id, params, signal) {
      ...
      if (input.run_in_background !== false) {
        if (options.allowBackground === false) {
          throw new Error('Background sub-agents are disabled in Yapi settings.');
        }
        if (!runner.spawn) {
