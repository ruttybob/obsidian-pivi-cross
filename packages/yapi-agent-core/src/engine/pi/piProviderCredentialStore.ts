...
function getLegacyPiAiCredentialSecretId(providerId: string): string {
  return `${YAPI_PROVIDER_SECRET_PREFIX}-${providerId}-${LEGACY_PI_AI_CREDENTIAL_KIND}`;
}
...
export class ObsidianCredentialStore implements CredentialStore {
  ...
  async list(): Promise<readonly CredentialInfo[]> {
    const secretIds = this.secretStorage.listSecrets(`${YAPI_PROVIDER_SECRET_PREFIX}-`);
    const infos: CredentialInfo[] = [];
    for (const secretId of secretIds) {
      const match = new RegExp(`^${YAPI_PROVIDER_SECRET_PREFIX}-(.+)-credential$`).exec(secretId);
      if (!match?.[1]) {
        continue;
      }
