...
import {
  ...
  parseAgentReport,
  parseYapiCompactionDetails,
  type YapiCompactionDetails,
...
import {
  ...
  YAPI_SESSION_META,
  YAPI_UI_CONTEXT,
  type YapiMessageUiData,
  type YapiSessionMetaData,
  type YapiUiContextData,
} from '../../../session/types';
...
function collectPersistedAsyncSubagentResults(
  ...
  for (const entry of entries) {
    if (entry.type !== 'custom' || entry.customType !== YAPI_MESSAGE_UI) {
      continue;
    }
    const data = entry.data as YapiMessageUiData | undefined;
...
export class SessionTreeStore {
  ...
  private rewriteToDisk(): void {
    ...
    // Yapi flushes earlier so history rows exist immediately, keep Pi's lazy
  ...
  static create(vaultPath: string): SessionTreeStore {
    ...
    // Pi normally creates the file on the first assistant response. Yapi needs
  ...
  /**
   * Linear model context view: Yapi restores sessions by append order, while
  ...
  appendCustomMeta(data: YapiSessionMetaData): string {
    this.assertWritableSource();
    const entryId = this.manager.appendCustomEntry(YAPI_SESSION_META, data);
    this.refreshIndexAfterAppend([entryId]);
    this.registerLive();
    return entryId;
  }

  appendUiContext(data: YapiUiContextData): string {
    this.assertWritableSource();
    const entryId = this.manager.appendCustomEntry(YAPI_UI_CONTEXT, data);
    this.refreshIndexAfterAppend([entryId]);
    this.registerLive();
    return entryId;
  }

  appendMessageUi(data: YapiMessageUiData): string {
    this.assertWritableSource();
    const { sanitized } = sanitizeMessageUiForJsonl(data);
    const entryId = this.manager.appendCustomEntry(YAPI_MESSAGE_UI, sanitized);
    this.refreshIndexAfterAppend([entryId]);
    this.registerLive();
    return entryId;
  }
  ...
  appendCompaction(
    ...
    tokensBefore: number,
    details?: YapiCompactionDetails,
  ): string {
    this.assertWritableSource();
    const validatedDetails = details ? parseYapiCompactionDetails(details) ?? undefined : undefined;
