...
import { parseYapiCompactionDetails } from '../../../session/continuationSchemas';
import {
  YAPI_MESSAGE_UI,
  YAPI_SESSION_META,
...
function mergeAssistantMessageSegment(
  ...
  segment: {
    ...
    ui: YapiMessageUiData | undefined;
  },
): void {
...
function messageUiFromCustom(data: unknown): YapiMessageUiData | null {
  if (!data || typeof data !== 'object') {
    return null;
  }
  const candidate = data as YapiMessageUiData;
...
export function entriesToChatMessages(
  branch: SessionEntry[],
  messageUiByEntryId: Map<string, YapiMessageUiData>,
): ChatMessage[] {
  ...
  for (let entryIndex = 0; entryIndex < branch.length; entryIndex += 1) {
    ...
    if (entry.type === 'compaction') {
      ...
      const details = parseYapiCompactionDetails(
      ...
      const message: ChatMessage = {
        ...
        contentBlocks: [{
          ...
          ...(details ? { checkpoint: toCheckpointPresentation(details.yapiCheckpoint) } : {}),
...
export function collectMessageUiMap(branch: SessionEntry[]): Map<string, YapiMessageUiData> {
  const map = new Map<string, YapiMessageUiData>();
  for (const entry of branch) {
    if (!isCustomEntry(entry) || entry.customType !== YAPI_MESSAGE_UI) {
      continue;
    }
...
export function readSessionMetaFromBranch(branch: SessionEntry[]): YapiSessionMetaData | null {
  for (let i = branch.length - 1; i >= 0; i--) {
    const entry = branch[i];
    if (!entry || !isCustomEntry(entry) || entry.customType !== YAPI_SESSION_META) {
      continue;
    }
    const data = entry.data as YapiSessionMetaData | undefined;
