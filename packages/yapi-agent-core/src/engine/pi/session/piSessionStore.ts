...
export function stripExternalContextsFromSessionJsonl(
  ...
  const migratedLines = lines.map((line, index) => {
    ...
    if (parsed.customType === YAPI_UI_CONTEXT && Object.hasOwn(data, 'externalContextPaths')) {
    ...
    if (parsed.customType === YAPI_MESSAGE_UI && typeof data.targetEntryId === 'string') {
...
function sessionMetaEqual(
  a: YapiSessionMetaData | null | undefined,
  b: YapiSessionMetaData,
): boolean {
  return !!a
    && a.title === b.title
...
export class PiSessionStore implements SessionStore {
  ...
  async migrateDeviceLocalExternalContexts(): Promise<number> {
    const files = (await this.adapter.listFilesRecursive('.yapi/sessions'))
    ...
    for (const absoluteFile of files) {
      try {
        ...
        const metaLine = [...index.entries].reverse().find(line => (
          line.customType === YAPI_SESSION_META
        ));
        const meta = metaLine
          ? (readSessionJsonlIndexedLine(index, metaLine).data as YapiSessionMetaData | undefined)
          : undefined;
  ...
  readUiContext(ref: SessionRef): Promise<SessionUiContext> {
    ...
    for (let i = index.entries.length - 1; i >= 0; i--) {
      const line = index.entries[i];
      if (line?.customType !== YAPI_UI_CONTEXT) continue;
      const entry = readSessionJsonlIndexedLine(index, line);
      const data = entry.data as YapiUiContextData | undefined;
      if (data) {
  ...
  async writeUiContext(
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
      ...
      if (entry?.type !== 'custom' || entry.customType !== YAPI_UI_CONTEXT) continue;
      const data = entry.data as YapiUiContextData | undefined;
      current = {
        currentNote: data?.currentNote,
  ...
  writeSessionMeta(ref: SessionRef, patch: SessionMetaPatch): Promise<void> {
    ...
    const next: YapiSessionMetaData = {
