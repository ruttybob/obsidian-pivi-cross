...
export function buildRegisteredToolsSection(summary: RegisteredToolSummary): string {
  ...
  if (summary.includeMcp) {
    lines.push(
      '',
      '### MCP',
      '- `mcp` — Vault MCP servers (.yapi/mcp.json). All settings-enabled servers are available; use search/list before calling tools.',
    );
  ...
  if (summary.includeSkill) {
    lines.push('', '### Skills', `- \`${TOOL_SKILL}\` — Load a vault skill by name from .yapi/skills/`);
  }
...
function buildApiVsCliGuidance(registeredObsidianTools: Set<string>, obsidianCliAvailable: boolean): string {
  ...
  if (!obsidianCliAvailable) {
    notes.push('Obsidian CLI is not available for this turn (disabled in Yapi settings or not enabled in Obsidian). Do not use CLI-only tools or CLI-only actions; use API-backed actions when listed. If the user’s request cannot be completed without a CLI-only tool/action (for example history restore, daily-note commands, command/eval, tasks, or base query), stop and ask the user to enable Yapi’s Obsidian CLI setting and Obsidian Settings → General → Command line interface, then retry.');
  }
