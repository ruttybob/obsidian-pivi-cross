...
import { YAPI_MCP_CONFIG_PATH } from "./paths";
...
export { YAPI_MCP_CONFIG_PATH } from "./paths";
...
function getMcpSecretId(serverName: string, kind: McpSecretKind): string {
  return `yapi-mcp-${encodeSecretName(serverName)}-${kind}`;
}
...
export class McpStorage {
  ...
  async save(servers: ManagedMcpServer[]): Promise<void> {
    ...
    if (await this.adapter.exists(YAPI_MCP_CONFIG_PATH)) {
      existing = await this.readJsonObject(YAPI_MCP_CONFIG_PATH);
    }
    ...
    const existingYapi =
      existing && typeof existing._yapi === "object"
        ? (existing._yapi as Record<string, unknown>)
        : null;

    if (Object.keys(yapiServers).length > 0) {
      file._yapi = { ...(existingYapi ?? {}), servers: yapiServers };
    } else if (existingYapi) {
      const rest = { ...existingYapi };
      delete rest.servers;
    ...
    await this.adapter.ensureFolder(".yapi");
  ...
  private async readConfigContent(): Promise<string | null> {
    if (await this.adapter.exists(YAPI_MCP_CONFIG_PATH)) {
      return this.adapter.read(YAPI_MCP_CONFIG_PATH);
    }
    return null;
  }
  ...
  private parseServers(file: ManagedMcpConfigFile): ManagedMcpServer[] {
    ...
    const yapiMeta = file._yapi?.servers ?? {};
