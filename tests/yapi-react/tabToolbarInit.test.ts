import type { ChatSettingsSnapshot } from '@yapi/yapi-agent-core/runtime/chatPorts';
...
function createToolbarTab(state: ChatState): TabData {
  return {
    ...
    sessionFile: '.yapi/sessions/session-1.jsonl',
    leafId: null,
...
describe('composer model usage limits', () => {
  it('updates the context limit before metadata preparation completes and refreshes it afterward', async () => {
    ...
    wireComposerChrome(tab, asYapiPlugin(createMockYapiPluginStub()), ports);
