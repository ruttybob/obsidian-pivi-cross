...
import { InlineEditView, createInlineEditController, hideInlineEditWidget, inlineEditWidgetField, mountInlineEdit, showInlineEditWidget, type MountedInlineEdit } from '@yapi/yapi-react';
import type { InlineEditPort } from '@yapi/yapi-react/ports';
import { I18nProvider } from '@yapi/yapi-react';
import type { InlineEditResult, InlineEditService } from '@yapi/yapi-agent-core/runtime/auxTypes';
...
describe('InlineEditView', () => {
  ...
  it('unmounts the React widget when CodeMirror removes its decoration', () => {
    ...
    expect(parent.querySelector('.yapi-inline-react-root')?.ownerDocument).toBe(ownerDocument);
    act(() => view.dispatch({ effects: hideInlineEditWidget.of(null) }));
    expect(parent.querySelector('.yapi-inline-react-root')).toBeNull();
    view.destroy();
    iframe.remove();
  });
});
