...
describe('React MCP settings', () => {
  it('uses a slash marker and reveals tools after opening the provider-style card', async () => {
    ...
    expect(document.querySelector('.yapi-mcp-container')?.lastElementChild).toContainElement(addButton);
    expect(screen.getByTitle('Slash badges: /remote tokens highlight this server in the composer')).toHaveTextContent('/');
    expect(document.querySelector('.yapi-mcp-context-saving-badge')).not.toHaveTextContent('@');
    expect(screen.getByText('2 tools')).toBeInTheDocument();
    fireEvent.click(screen.getByText('remote', { selector: '.yapi-mcp-name' }));
    await act(async () => undefined);
    expect(screen.getByText('read', { selector: '.yapi-mcp-tool-name' })).toBeInTheDocument();
    expect(screen.getByText('search', { selector: '.yapi-mcp-tool-name' })).toBeInTheDocument();
  ...
  it('keeps enable and remove actions in the header without opening the card', async () => {
    ...
    const card = document.querySelector('.yapi-mcp-card') as HTMLDetailsElement;
    const header = card.querySelector('.yapi-mcp-card-header');
  ...
  it('validates, creates, edits, deletes, and imports MCP servers', async () => {
    ...
    fireEvent.click(screen.getByText('local', { selector: '.yapi-mcp-name' })); await act(async () => undefined); expect(screen.queryByRole('button', { name: 'Edit' })).not.toBeInTheDocument(); const inlineEditor = document.querySelector('.yapi-mcp-inline-editor'); expect(inlineEditor).toContainElement(screen.getByRole('button', { name: 'Connect / refresh tools' })); expect(screen.queryByRole('button', { name: 'Save' })).not.toBeInTheDocument(); fireEvent.change(screen.getByPlaceholderText('my-mcp-server'), { target: { value: 'renamed' } }); fireEvent.click(screen.getByRole('button', { name: 'Connect / refresh tools' })); await act(async () => undefined); expect(getServers()[0]?.name).toBe('renamed'); expect(mcp.connect).toHaveBeenCalledWith(expect.objectContaining({ name: 'renamed' }));
  ...
  it('saves and connects an OAuth server from one primary action', async () => {
    ...
    fireEvent.click(screen.getByText('remote', { selector: '.yapi-mcp-name' })); await act(async () => undefined); expect(screen.queryByRole('button', { name: 'OAuth' })).not.toBeInTheDocument(); fireEvent.click(screen.getByRole('button', { name: 'Connect / refresh tools' })); await act(async () => undefined); expect(mcp.save).toHaveBeenCalled(); expect(mcp.connect).toHaveBeenCalledWith(server);
  });
  ...
  it('does not report an auth error when a public MCP needs no OAuth', async () => {
    ...
    fireEvent.click(screen.getByText('deepwiki', { selector: '.yapi-mcp-name' }));
    fireEvent.click(screen.getByRole('button', { name: 'Connect / refresh tools' }));
    await act(async () => undefined);
