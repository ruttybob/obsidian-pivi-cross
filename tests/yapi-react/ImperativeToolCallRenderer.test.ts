...
import { TOOL_BASH } from '@yapi/yapi-agent-core/tools/toolNames';
...
describe('imperative stored tool-call header', () => {
  it('uses the shared translated title, result-aware summary, class, and aria label', () => {
    ...
    const header = toolElement.querySelector('.yapi-tool-header');

    expect(toolElement).toHaveClass('yapi-tool-call-obsidian');
    expect(toolElement.querySelector('.yapi-tool-name')).toHaveTextContent('Search');
    expect(toolElement.querySelector('.yapi-tool-summary'))
  ...
  it('applies descriptor-owned shell classes to imperative rows', () => {
    ...
    expect(toolElement).toHaveClass('yapi-tool-call-bash');
    expect(toolElement.querySelector('.yapi-tool-header'))
  ...
  it('renders long plain output completely with a constant-size text body', () => {
    ...
    expect(toolElement.querySelector('.yapi-tool-content')).toBeEmptyDOMElement();
    (toolElement.querySelector('.yapi-tool-header') as HTMLElement).click();

    const lines = toolElement.querySelectorAll('.yapi-tool-lines > .yapi-tool-line');
  ...
  it('defers collapsed updates and renders only the latest snapshot on re-expand', () => {
    ...
    const header = toolElement.querySelector('.yapi-tool-header') as HTMLElement;

    header.click();
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('first result');
    header.click();
    updateToolCallElement(toolElement, { ...initial, result: 'latest result' });
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('first result');

    header.click();
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('latest result');
    expect(toolElement.querySelector('.yapi-tool-content')).not.toHaveTextContent('first result');
  });
  ...
  it('preserves source-owned truncation metadata without adding a UI preview cap', () => {
    ...
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('available first line');
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('available last line');
    expect(toolElement.querySelector('.yapi-tool-source-truncation'))
  ...
  it('renders completed Markdown reads through the injected Obsidian renderer', async () => {
    ...
    expect(toolElement.querySelector('.yapi-tool-read-markdown h1')).toHaveTextContent('Heading');
    expect(toolElement.querySelector('.yapi-tool-read-markdown h1')).toHaveTextContent('line 39');
  });
});
...
describe('Obsidian outline preview', () => {
  ...
  it('dispatches the outline tool to the YAML renderer', () => {
    ...
    expect(toolElement.querySelector('.yapi-tool-content')).toHaveTextContent('headings: []');
  });
});
