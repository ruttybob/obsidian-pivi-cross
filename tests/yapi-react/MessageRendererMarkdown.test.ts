...
describe('Markdown code block enhancement', () => {
  ...
  it('leaves Obsidian frontmatter placeholders hidden', async () => {
    ...
    expect(container.querySelector('.yapi-code-wrapper')).toBeNull();
    expect(container.querySelector('.yapi-code-lang-label')).toBeNull();
  });
  ...
  it('keeps enhancing explicit fenced YAML blocks', async () => {
    ...
    expect(container.querySelector('.yapi-code-wrapper--language')).not.toBeNull();
    expect(container.querySelector('.yapi-code-lang-label')).toHaveTextContent('yaml');
  });
});
...
describe('restored user message context', () => {
  it('supplements only the first-turn auto-attached current-note badge', async () => {
    ...
    const badges = [...container.querySelectorAll<HTMLElement>('.yapi-context-badge')];
  ...
  it('renders a folder from the input without listing its expanded context files', async () => {
    ...
    const badges = [...container.querySelectorAll<HTMLElement>('.yapi-context-badge')];
    expect(badges).toHaveLength(1);
    expect(badges[0]).toHaveClass('yapi-context-badge-kind-folder');
    ...
    expect(container.querySelector('.yapi-user-context-badges')).toBeNull();
  });
});
