...
import { createI18n, I18nProvider, MessageList } from '@yapi/yapi-react';
import { type ChatPerfRecorder, ChatProjectionStore } from '@yapi/yapi-react/store';
...
describe('MessageList', () => {
  it('renders older paging as a Memory boundary', () => {
    ...
    expect(screen.getByRole('separator', { name: 'Earlier history' })).toHaveClass(
      'yapi-memory-boundary',
      'yapi-history-boundary',
    );
  });
  ...
  it('keeps a 5K transcript mounted row count bounded by the viewport and overscan', () => {
    ...
    const mountedRows = rendered.container.querySelectorAll('.yapi-message-virtual-row').length;
  ...
  it('remeasures only the row whose subscribed block grows', () => {
    ...
    try {
      ...
      const rows = [...rendered.container.querySelectorAll('.yapi-message-virtual-row')];
      ...
      expect(rendered.container.querySelector('.yapi-message-list')).toHaveStyle({ height: '360px' });
    } finally {
  ...
  it('keeps historical toolbars but hides the active turn toolbars until streaming stops', () => {
    ...
    expect(rendered.container.querySelector('[data-message-id="user-1"] .yapi-message-actions')).not.toBeNull();
    expect(rendered.container.querySelector('[data-message-id="assistant-1"] .yapi-message-actions')).not.toBeNull();
    expect(rendered.container.querySelector('[data-message-id="user-current"] .yapi-message-actions')).toBeNull();
    expect(rendered.container.querySelector('[data-message-id="assistant-current"] .yapi-message-actions')).toBeNull();
    ...
    expect(rendered.container.querySelector('[data-message-id="user-current"] .yapi-message-actions')).not.toBeNull();
    expect(rendered.container.querySelector('[data-message-id="assistant-current"] .yapi-message-actions')).not.toBeNull();
  });
  ...
  it('wraps user images, opens the modal, and closes via overlay, close, and Escape', () => {
    ...
    const wrapper = document.querySelector('.yapi-message-images .yapi-message-image img');
    expect(wrapper).not.toBeNull();
    fireEvent.click(wrapper!);
    expect(document.querySelector('.yapi-image-modal-overlay .yapi-image-modal img')).not.toBeNull();

    fireEvent.click(document.querySelector('.yapi-image-modal-close')!);
    expect(document.querySelector('.yapi-image-modal-overlay')).toBeNull();

    fireEvent.click(wrapper!);
    fireEvent.click(document.querySelector('.yapi-image-modal-overlay')!);
    expect(document.querySelector('.yapi-image-modal-overlay')).toBeNull();
  ...
  it('hides empty assistants, shows stored interrupt indicators, and marks tool-only shells', () => {
    ...
    expect(container.querySelector('[data-message-id="interrupt"] .yapi-interrupted')).not.toBeNull();
    expect(container.querySelector('[data-message-id="interrupt-content"] .yapi-interrupted')).not.toBeNull();
    expect(container.querySelector('[data-message-id="interrupt-content"]')).toHaveTextContent('Partial answer');
    expect(container.querySelector('[data-message-id="tool-only"]')).toHaveClass('yapi-message-assistant-tool-only');
    // No stored isInterrupt => React must not append a second interrupt indicator shell.
    expect(container.querySelector('[data-message-id="live-cancel"] .yapi-interrupted')).toBeNull();
