...
describe('MentionDropdownController folder labels', () => {
  ...
  it('omits the redundant @ in the dropdown but keeps it in the inserted token', () => {
    ...
    expect(container.querySelector('.yapi-mention-name-folder')).toHaveTextContent('notes/');
    expect(container.querySelector('.yapi-mention-name-folder')).not.toHaveTextContent('@notes/');
  ...
  it('localizes the empty state', () => {
    ...
    expect(container.querySelector('.yapi-mention-empty')).toHaveTextContent('没有匹配项');
    controller.destroy();
  });
});
