...
import type { ActivityStatus, ToolCallInfo } from '@yapi/yapi-agent-core/foundation';
import {
  TOOL_OBSIDIAN_EDIT,
...
import { DisclosureAnchorProvider } from '../../packages/yapi-react/src/chat/messages/disclosureAnchorContext';
...
describe('ToolCallView', () => {
  ...
  it('renders long fallback text with one body node after expansion', () => {
    ...
    const body = view.container.querySelector('.yapi-tool-raw-text');
  ...
  it('keeps repeated tool inputs as distinct, ordered rows with raw tool names', () => {
    ...
    expect(document.querySelectorAll('.yapi-tool-call-in-step-group .yapi-tool-name')).toHaveLength(2);
    expect(document.querySelectorAll('.yapi-tool-call-in-step-group')).toHaveLength(2);
  ...
  it('renders every localized activity state and animates only running', () => {
    ...
    expect(statuses.map(status => document.querySelector(`.yapi-tool-status.status-${status}`)?.textContent))
      .toEqual(['Queued', 'Running', 'Waiting', 'Completed', 'Failed', 'Cancelled', 'Orphaned']);
    expect(document.querySelectorAll('.yapi-working-icon-arc')).toHaveLength(1);
    expect(document.querySelectorAll('.yapi-tool-status[aria-live="polite"]')).toHaveLength(7);
    expect(document.querySelector('.yapi-tool-status.status-orphaned')).toHaveAttribute(
      'aria-label',
  ...
  it('updates the polite status region at phase and terminal transitions', () => {
    ...
    const liveRegion = () => view.container.querySelector('.yapi-activity-status[aria-live="polite"]');
  ...
  it('uses the row owner window for elapsed time and freezes at completion', () => {
    ...
    expect(ownerDocument.querySelector('.yapi-activity-elapsed')).toHaveTextContent('2s');
    jest.setSystemTime(11_000);
    act(() => tick?.());
    expect(ownerDocument.querySelector('.yapi-activity-elapsed')).toHaveTextContent('3s');
    ...
    act(() => tick?.());
    expect(ownerDocument.querySelector('.yapi-activity-elapsed')).toHaveTextContent('3s');
    expect(ownerDocument.querySelector('.yapi-activity-elapsed')).toHaveAttribute('aria-hidden', 'true');
  ...
  it('uses one generic shell for Edit and keeps diff stats in its header', () => {
    ...
    expect(document.querySelectorAll('.yapi-tool-call')).toHaveLength(1);
    expect(document.querySelectorAll('.yapi-tool-header')).toHaveLength(1);
    expect(document.querySelector('.yapi-write-edit-block')).not.toBeInTheDocument();
    expect(document.querySelector('.yapi-write-edit-header')).not.toBeInTheDocument();
    expect(document.querySelector('.yapi-write-edit-stats')).toHaveTextContent('+1');
  ...
  it('renders MCP proxy calls with the MCP icon and server/tool title', () => {
    ...
    expect(document.querySelector('.yapi-tool-icon svg title')).toHaveTextContent('MCP');
    expect(document.querySelector('.yapi-tool-name')).toHaveTextContent('exa/search');
  ...
  it('renders collapsible chevrons and visible step-group latest summary', () => {
    const tool = toolCall('bash-1', TOOL_BASH, 'completed', { command: 'pwd' }, 'workspace');
    renderTool(<ToolCallView toolCall={tool} />);
    const individualChevron = document.querySelector('.yapi-tool-header .yapi-collapsible-chevron');
    expect(individualChevron).toHaveClass('is-collapsed');
    fireEvent.click(screen.getByRole('button', { name: 'Bash: pwd' }));
    expect(document.querySelector('.yapi-tool-call')).toHaveClass('expanded');
    expect(document.querySelector('.yapi-tool-header .yapi-collapsible-chevron')).not.toHaveClass('is-collapsed');
  });
  ...
  it('keeps descriptor-owned Bash and web CSS classes', () => {
    ...
    expect(document.querySelector('[data-tool-id="bash-class"]')).toHaveClass('yapi-tool-call-bash');
    expect(document.querySelector('[data-tool-id="web-class"]')).toHaveClass('yapi-tool-call-web');
  });
  ...
  it('shows step-group latest summary and rotates the group chevron', () => {
    ...
    renderTool(<ToolStepGroupView toolCalls={[first, second]} />);
    expect(document.querySelector('.yapi-tool-step-group-summary')?.textContent).toBeTruthy();
    expect(document.querySelector('.yapi-tool-step-group-status')).toHaveTextContent('2 Completed');
    const groupChevron = document.querySelector('.yapi-tool-step-group-header .yapi-collapsible-chevron');
    expect(groupChevron).toHaveClass('is-collapsed');
    fireEvent.click(screen.getByRole('button', { name: /2 steps/ }));
    expect(document.querySelector('.yapi-tool-step-group')).toHaveClass('expanded');
    expect(document.querySelector('.yapi-tool-step-group-header .yapi-collapsible-chevron')).not.toHaveClass('is-collapsed');
  });
  ...
  it('rerenders only the projected tool whose entity changes', () => {
    ...
    expect(document.querySelector('[data-tool-id="bash-1"] .yapi-tool-status')).toHaveClass('status-completed');
    expect(document.querySelector('[data-tool-id="bash-2"] .yapi-tool-status')).toHaveClass('status-running');
  });
  ...
  it('updates projected group status counts when any member changes', () => {
    ...
    const initialStatus = document.querySelector('.yapi-tool-step-group-status');
    ...
    const completedStatus = document.querySelector('.yapi-tool-step-group-status');
    expect(completedStatus).toHaveTextContent('2 Completed');
    expect(completedStatus?.querySelectorAll('.yapi-tool-status')).toHaveLength(1);
  ...
  it('shows completed and failed step counts separately', () => {
    ...
    const status = document.querySelector('.yapi-tool-step-group-status');
  ...
  it('updates an imperative projected tool adapter without remounting it', () => {
    ...
    expect(document.querySelector('.yapi-tool-content-adapter')).toHaveTextContent('completed');
  });
