...
import { createI18n, I18nProvider, SettingsRoot, SettingsUiStore } from '@yapi/yapi-react';
...
function createModelsPort() {
  return {
    ...
    getSecretId: (id: string) => `yapi-${id}-credential`,
...
function createPorts(overrides: Partial<SettingsPorts['actions']> = {}): SettingsPorts {
  return {
    ...
    hotkeys: {
      listHotkeys: () => [
        { commandId: 'yapi:open-view', labelKey: 'settings.openChatHotkey.name', hotkey: 'Mod+P' },
      ],
...
describe('React settings foundation', () => {
  ...
  it('applies a language change immediately and renders tabs without duplicate page headings', async () => {
    ...
    expect(screen.getByRole('combobox', { name: '语言' })).toHaveClass('yapi-select');
  ...
  it('normalizes excluded tags into removable badges', async () => {
    ...
    expect(screen.getByText('private', { selector: '.yapi-settings-badge__text' })).toBeInTheDocument();
  ...
  it('uses the shared Settings control style without applying it to toggles or ranges', () => {
    ...
    for (const control of container.querySelectorAll('input:not([type="checkbox"]):not([type="range"]), textarea, select')) {
      expect(control).toHaveClass('yapi-settings-control');
    }
    expect(container.querySelector('input[type="checkbox"]')).not.toHaveClass('yapi-settings-control');
    expect(container.querySelector('input[type="range"]')).not.toHaveClass('yapi-settings-control');
  });
  ...
  it('renders and runs host-provided integration sections', async () => {
    ...
    ports.hostIntegrations = {
      listSections: async () => [{
        ...
        description: 'Connect Yapi to the note host.',
    ...
    expect(integrationsHeading).toHaveClass('yapi-settings-section-heading');
    await screen.findByText('Host extension');
    const integrationSetting = container.querySelector<HTMLElement>('.yapi-integration-setting.yapi-setting-stack');
    expect(integrationSetting).not.toBeNull();
    expect(within(integrationSetting!).getByText('Host extension')).toHaveClass('yapi-setting-row__name');
    expect(within(integrationSetting!).getByText('Connect Yapi to the note host.')).toBeInTheDocument();
    ...
    expect(integrationSetting!.querySelector('.yapi-setting-row__name:empty')).toBeNull();
  ...
  it('lists remote skills and installs selected skills', async () => {
    ...
    const remoteSetting = container.querySelector<HTMLElement>('.yapi-skills-remote-setting');
    const installedHeader = screen.getByText('Installed skills').closest('.yapi-settings-list-header');
    expect(remoteSetting).not.toBeNull();
    expect(installedHeader).not.toBeNull();
    expect(installedHeader).toHaveClass('yapi-settings-list-header');
    expect(remoteSetting!.querySelector('.yapi-setting-row')).toBeInTheDocument();
  ...
  it('orders Local, OAuth, API, and Custom API groups and keeps added OAuth providers visible', () => {
    ...
    const sections = [...container.querySelectorAll('.yapi-provider-add-section')]
  ...
  it('reorders model providers with the keyboard while retaining provider icons', async () => {
    ...
    expect(container.querySelectorAll('.yapi-model-provider-card .yapi-provider-logo-mask')).toHaveLength(2);
  ...
  it('calls provider credential and Codex OAuth ports from an expanded card', async () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: 'Models' }));
    const credentialSetting = container.querySelector<HTMLElement>('.yapi-cred-row');
  ...
  it('keeps Codex actions below the sign-in guidance', () => {
    ...
    const codexSetting = container.querySelector<HTMLElement>('.yapi-provider-oauth-setting');
  ...
  it('calls Grok Build OAuth connect from an expanded provider card', async () => {
    ...
    render(withTestPresentationPlatform(<I18nProvider i18n={createI18n()}><SettingsRoot ports={ports} initialTab="models" /></I18nProvider>));
    fireEvent.click(screen.getByText('Grok Build', { selector: '.yapi-provider-title' }));
    fireEvent.click(screen.getByRole('button', { name: 'Connect' }));
    await act(async () => undefined);
  ...
  it('renders Anthropic API key controls without manual OAuth token UI', async () => {
    ...
    expect(container.querySelector('.yapi-cred-row')).not.toBeNull();
  ...
  it('renders Claude OAuth connect on the Claude provider card', async () => {
    ...
    render(withTestPresentationPlatform(<I18nProvider i18n={createI18n()}><SettingsRoot ports={ports} initialTab="models" /></I18nProvider>));
    fireEvent.click(screen.getByText('Claude', { selector: '.yapi-provider-title' }));
    fireEvent.click(screen.getByRole('button', { name: 'Connect' }));
    await act(async () => undefined);
