import { createDisclosureViewportController } from '../../packages/yapi-react/src/chat/messages/disclosureViewport';
...
describe('disclosure viewport controller', () => {
  ...
  function createFixture(height = 600) {
    ...
    row.className = 'yapi-message-virtual-row';
  ...
  it('tracks one third of the owner viewport and restores the prior style on dispose', () => {
    ...
    scrollElement.setCssProps({ '--yapi-expanded-content-max-height': '111px' });
    const controller = createDisclosureViewportController(scrollElement);

    expect(scrollElement.style.getPropertyValue('--yapi-expanded-content-max-height')).toBe('200px');
    expect(scrollElement.style.getPropertyValue('--yapi-subagent-expanded-max-height')).toBe('400px');
    ...
    expect(scrollElement.style.getPropertyValue('--yapi-expanded-content-max-height')).toBe('300px');
    expect(scrollElement.style.getPropertyValue('--yapi-subagent-expanded-max-height')).toBe('600px');

    controller.dispose();
    expect(scrollElement.style.getPropertyValue('--yapi-expanded-content-max-height')).toBe('111px');
    expect(scrollElement.style.getPropertyValue('--yapi-subagent-expanded-max-height')).toBe('');
  ...
  it('compensates repeated virtual-row growth without moving the activated header', () => {
    ...
    expect(
      Number.parseFloat(
        scrollElement.style.getPropertyValue('--yapi-expanded-content-max-height'),
      ),
    ).toBeCloseTo(739 / 3);
    expect(
      Number.parseFloat(
        scrollElement.style.getPropertyValue('--yapi-subagent-expanded-max-height'),
      ),
    ).toBeCloseTo((739 * 2) / 3);
  ...
  function createDisclosureFixture(
    ...
    if (nestedInSubagent) {
      ...
      subagent.className = 'yapi-subagent-list expanded';
      const subagentContent = document.createElement('div');
      subagentContent.className = 'yapi-subagent-content';
  ...
  it('never mutates disclosure state when inner scrolling reaches its end', () => {
    const { row, scrollElement } = createFixture();
    const wrapper = document.createElement('div');
    wrapper.className = 'yapi-tool-call expanded';
    const header = document.createElement('button');
    header.className = 'yapi-tool-header';
  ...
  it('adjusts the inner subagent scrollport when a nested sticky tool header collapses', () => {
    ...
    group.className = 'yapi-tool-step-group expanded';
    const steps = document.createElement('div');
    steps.className = 'yapi-tool-step-group-steps';
    const stepItem = document.createElement('div');
    stepItem.className = 'yapi-tool-step-item';
    const tool = document.createElement('div');
    tool.className = 'yapi-tool-call expanded yapi-tool-call-in-step-group';
    const header = document.createElement('button');
    header.className = 'yapi-tool-header';
  ...
  it('publishes each expanded steps header height as its child sticky offset', () => {
    ...
    group.className = 'yapi-tool-step-group expanded';
    const header = document.createElement('button');
    header.className = 'yapi-tool-step-group-header';
    header.getBoundingClientRect = jest.fn(() => ({
      bottom: 19,
      height: 19,
      left: 0,
    ...
    expect(group.style.getPropertyValue('--yapi-tool-step-group-sticky-top')).toBe('19px');
    controller.dispose();
    expect(group.style.getPropertyValue('--yapi-tool-step-group-sticky-top')).toBe('');
  });
  ...
  it('does not shrink or expand wrappers on wheel after internal scroll end', () => {
    const { body, header, scrollElement, wrapper } = createDisclosureFixture(
      'yapi-subagent-list',
      'yapi-subagent-header',
      'yapi-subagent-content',
    );
    ...
    expect(wrapper.classList.contains('yapi-disclosure-chain-active')).toBe(false);
    expect(wrapper.style.getPropertyValue('--yapi-disclosure-chain-max-height')).toBe('');
    expect(wrapper.classList.contains('expanded')).toBe(true);
    controller.dispose();
  });
