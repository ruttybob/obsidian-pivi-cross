...
import { ActiveChatUiBridge, mountChatView } from '@yapi/yapi-react/mount';
import {
  ChatTabsStore,
  ChatProjectionStore,
  ChatUiStore,
...
describe('React ChatShell tabs', () => {
  ...
  it('projects state classes and delegates switch, create, rename, archive, and close actions', async () => {
    ...
    expect(document.querySelector('.yapi-tab-switcher-item[data-tab-id="attention"] .is-unread')).not.toBeNull();
  ...
  it('keeps the menu mounted for its close animation for trigger and outside clicks', async () => {
    ...
    expect(mounted.host.querySelector('.yapi-tab-switcher')).not.toHaveClass('is-open');
    expect(mounted.host.querySelector('.yapi-tab-switcher-menu')).toHaveClass('is-closing');
    act(() => jest.advanceTimersByTime(279));
    expect(mounted.host.querySelector('.yapi-tab-switcher-menu')).not.toBeNull();
    act(() => jest.advanceTimersByTime(1));
    expect(mounted.host.querySelector('.yapi-tab-switcher-menu')).toBeNull();
  ...
  it('animates active title changes in tab-index direction', async () => {
    ...
    let title = mounted.host.querySelector('.yapi-tab-switcher-title');
    ...
    title = mounted.host.querySelector('.yapi-tab-switcher-title');
  ...
  it('projects active chat state into dedicated welcome, queue, usage, todo, and navigation portals', async () => {
    ...
    act(() => uiStore.update({
      thinkingIndicator: {
        className: 'yapi-thinking',
        elapsedLabel: ' (esc to interrupt · 0:03)',
        text: 'Distilling...',
      },
    }));
    const thinkingIndicator = targets.messages.querySelector('.yapi-thinking');
    expect(thinkingIndicator).toHaveClass('yapi-response-meta');
    ...
    expect(targets.composer.querySelector('path.yapi-meter-bg')?.getAttribute('d')).toBe('M 1.94 11.5 A 7 7 0 1 1 14.06 11.5');
    expect(within(targets.composer).queryByLabelText(/Output /)).toBeNull();
    expect(targets.composer.querySelector('.yapi-context-meter-gauge-output')).toBeNull();
    expect(targets.composer.querySelectorAll('.yapi-context-meter-gauge')).toHaveLength(1);
    expect(targets.composer.querySelector('.yapi-input-action-group .yapi-context-meter')).not.toBeNull();
    ...
    expect(targets.composer.querySelector('.yapi-context-meter')).toBeNull();
    ...
    expect(targets.todo.querySelector('.yapi-status-panel-content')).toBeNull();
  ...
  it('delegates React composer actions through the active tab bridge', async () => {
    ...
    expect(targets.composer.querySelector('.yapi-external-context-count')?.textContent).toBe('1');
    expect(targets.composer.querySelector('.yapi-mcp-selector')).toBeNull();
    fireEvent.mouseEnter(targets.composer.querySelector('.yapi-external-context-selector')!);
    ...
    const toolbarChildren = [...targets.composer.querySelector('.yapi-input-toolbar')!.children];
    expect(toolbarChildren.map(element => element.className)).toEqual([
      'yapi-model-selector',
      'yapi-thinking-selector',
      'yapi-external-context-selector',
      'yapi-mode-selector',
      'yapi-input-action-group',
    ]);
    expect(targets.composer.querySelector('.yapi-model-btn .yapi-provider-logo-mask')).not.toBeNull();
    expect(targets.composer.querySelector('.yapi-toolbar-label-sizer')).toBeNull();
    expect(targets.composer.querySelector('.yapi-model-btn .yapi-model-label')?.textContent).toBe('Model A');
    expect(targets.composer.querySelector('.yapi-thinking-current .yapi-thinking-label')?.textContent).toBe('Low');
    // Composer chrome owns the toolbar portal; labels use natural width (no fixed sizer).
    expect(targets.composer.querySelector('.yapi-input-toolbar')).not.toBeNull();

    const modelSelector = targets.composer.querySelector('.yapi-model-selector')!;
    const modelTrigger = within(targets.composer).getByRole('button', { name: 'Model' });
    const modelDropdown = targets.composer.querySelector('.yapi-model-dropdown')!;
    ...
    const thinkingGears = targets.composer.querySelector('.yapi-thinking-gears')!;
    const thinkingTrigger = within(targets.composer).getByRole('button', { name: 'Reasoning' });
    const thinkingOptions = targets.composer.querySelector('.yapi-thinking-options')!;
    ...
    expect(targets.composer.querySelector('.yapi-model-btn .yapi-model-label')?.textContent).toBe('Longer Model B Name');
    expect(targets.composer.querySelector('.yapi-thinking-current .yapi-thinking-label')?.textContent).toBe('High');
    expect(targets.composer.querySelector('.yapi-toolbar-label-slot')).toBeNull();
