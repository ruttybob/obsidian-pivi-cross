import type { MountedSurface, MountChatViewOptions } from '@yapi/yapi-react/mount';
...
import { NOOP_CHAT_PERF_RECORDER } from '@yapi/yapi-react/store';
...
function createHandle(): YapiChatViewHandle {
  return {
    commands: {
      getState: jest.fn(() => ({
        mounted: true,
...
function createHarness() {
  ...
  const workspace = { id: 'workspace' } as unknown as YapiPluginWorkspace;
  ...
  const view = new YapiViewHost(leaf, plugin as never, getWorkspace);
...
describe('YapiViewHost shell lifecycle', () => {
  beforeAll(() => {
    Object.defineProperty(YapiViewHost.prototype, 'load', {
      configurable: true,
  ...
  it('does not mount after the view closes while workspace readiness is pending', async () => {
    let resolveWorkspace!: (workspace: YapiPluginWorkspace) => void;
    const readiness = new Promise<YapiPluginWorkspace>((resolve) => {
