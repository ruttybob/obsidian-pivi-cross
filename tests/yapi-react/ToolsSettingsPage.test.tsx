...
describe('React tools settings', () => {
  it('renders all tool sections in their fixed document order', async () => {
    ...
    expect(Array.from(container.querySelectorAll('.yapi-tools-settings-page > .yapi-settings-section')).map((section) => section.getAttribute('aria-labelledby')))
  ...
  it('stacks external directory controls below their description', () => {
    const { container } = renderTools(createPorts());
    const setting = container.querySelector<HTMLElement>('.yapi-external-directories-setting.yapi-setting-stack');
  ...
  it('delegates host tool availability changes through the tools port', async () => {
    ...
    expect(hostToolToggle.parentElement).toHaveClass('yapi-toggle');
  ...
  it('adds, deduplicates, and removes Bash command badges', async () => {
    ...
    expect(screen.getByText('npm run build', { selector: '.yapi-settings-badge__text' })).toBeInTheDocument();
  ...
  it('tracks a pointer drag and persists the previewed provider order on release', async () => {
    ...
    for (const card of cards) {
      card.getBoundingClientRect = jest.fn(() => {
        ...
        const dragOffset = Number.parseFloat(card.style.getPropertyValue('--yapi-provider-drag-y')) || 0;
  ...
  it('renders provider brand icons and keeps reorder announcements out of visual layout', () => {
    ...
    expect(container.querySelectorAll('.yapi-web-provider-card .yapi-provider-logo-mask')).toHaveLength(4);
    expect(container.querySelector('[aria-live="polite"]')).toHaveClass('yapi-visually-hidden');
  });
});
