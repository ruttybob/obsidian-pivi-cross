...
describe("Yapi view activation", () => {
  it("awaits revealing an existing deferred view", async () => {
    ...
    const activation = activateYapiView(app, "right-sidebar");
    let settled = false;
  ...
  it("resolves a view that becomes available while reveal completes", async () => {
    ...
    await expect(ensureYapiViewOpen(app, "right-sidebar")).resolves.toBe(view);
  });
  ...
  it("does not stack a blank tab on a cold open without restored tabs", async () => {
    ...
    await openYapiNewTab(app, "right-sidebar", {
      activeTabId: null,
      openTabs: [],
    });
