import type { AgentEngine } from '@yapi/yapi-agent-core/engine';
...
import type {
  ...
  SecretStore,
  WorkspaceFileStore,
} from '@yapi/yapi-agent-core/ports';
import type { AgentCoreHost } from '@yapi/yapi-agent-core/runtime/agentCoreHost';
import type { ChatTurnMetadata } from '@yapi/yapi-agent-core/runtime/types';
...
export function createCliProjectLikeHost(overrides?: FakeHostPortOverrides): AgentCoreHost {
  const workspace: WorkspaceContext = {
    ...
    name: 'yapi-cli',
    kind: 'cli-project',
    rootUri: 'file:///Users/dev/yapi-cli',
    yapiDir: '.yapi',
  };
  const files = createInMemoryFileStore({ 'package.json': '{"name":"yapi-cli"}' });
  const logger: Logger = {
    debug: jest.fn(),
    info: jest.fn(),
    warn: jest.fn(),
