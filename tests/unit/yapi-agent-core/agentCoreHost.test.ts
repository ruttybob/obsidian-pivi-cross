...
describe('AgentCoreHost contract', () => {
  ...
  it('lets a cli-project host use different port implementations for the same contract', async () => {
    ...
    expect(await host.files.read('package.json')).toBe('{"name":"yapi-cli"}');

    const listed = await host.sessions.listSessions('/Users/dev/yapi-cli');
    expect(sessions.listedVaultPath).toBe('/Users/dev/yapi-cli');
    const [session] = listed;
    expect(session).toBeDefined();
