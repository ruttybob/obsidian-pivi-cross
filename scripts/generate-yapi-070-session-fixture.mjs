...
const SYNTHETIC_VAULT = '/tmp/yapi-0.7.0-tag-generated-vault';
...
if (!outputArg) {
  fail('Usage: node scripts/generate-yapi-070-session-fixture.mjs <output-jsonl>');
}

const outputFile = resolve(outputArg);
const temporaryRoot = mkdtempSync(join(tmpdir(), 'yapi-0.7.0-source-'));
...
try {
  ...
  const preloadFile = join(worktreeRoot, 'yapi-0.7.0-deterministic-preload.mjs');
  ...
  const generatorFile = join(
    ...
    'packages',
    'yapi-agent-core',
    'yapi-0.7.0-fixture-generator.ts',
  );
  writeFileSync(generatorFile, String.raw`
    ...
    await store.writeUiContext(ref, {
      ...
      externalContextPaths: ['/synthetic/yapi-0.7.0/session-context'],
      enabledMcpServers: ['synthetic-mcp'],
    });
    ref = await store.appendUserTurn(ref, 'Hello from the Yapi 0.7.0 writer.', {
      displayContent: 'Hello from the Yapi 0.7.0 writer.',
      turnRequest: {
        text: 'Hello from the Yapi 0.7.0 writer.',
        currentNotePath: 'Synthetic/Provenance.md',
        externalContextPaths: ['/synthetic/yapi-0.7.0/turn-context'],
      },
    });
    ref = await store.appendAgentTurn(ref, [
      { role: 'user', content: 'Hello from the Yapi 0.7.0 writer.', timestamp: 1_720_047_600_100 },
      {
        role: 'assistant',
  ...
  if (!text.includes(SYNTHETIC_VAULT)
    || !text.includes('/synthetic/yapi-0.7.0/session-context')
    || !text.includes('/synthetic/yapi-0.7.0/turn-context')) {
