...
flowchart LR
  ...
  Parser --> Badges["@yapi/yapi-react/context-badges<br/>display model"]
...
- Current canonical MCP syntax is `/server` or `/server/tool`, not an `@` token. Selector labels omit the decorative `/`, while insertion and persistence retain `/server[/tool]`; the detail panel identifies the server. Slash badges still use known context-saving server names for composer highlighting; settings-enabled servers are always available to the agent. Turn finalization in `@yapi/yapi-agent-core` appends ` MCP` to valid slash tokens for the API prompt while persisted text stays unchanged.
...
- Keep dependencies pointing toward host-neutral YaPi contracts (`foundation`, `context`, `skills`, etc.). Never import `@yapi/yapi-agent-core/engine/pi`, raw Pi SDKs, `@yapi/obsidian-host`, or `src/app/workspace/**` here. Use injected structural providers/callbacks; use `src/app/hostPlatform.ts` only for the approved host-platform facade.
...
- Imperative dropdown DOM consumed by `@yapi/yapi-react` styles uses semantic `yapi-*` classes such as `yapi-mention-item--workspace-folder`; do not expose vault/host terminology or unnamespaced state classes across that presentation seam.
