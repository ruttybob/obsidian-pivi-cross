...
export function renderWriteEditContent(container: HTMLElement, toolCall: ToolCallInfo): void {
  container.addClass('yapi-write-edit-content');
  const row = container.createDiv({ cls: 'yapi-write-edit-diff-row' });
  const isError = toolCall.status === 'error' || toolCall.status === 'blocked';

  if (toolCall.diffData?.diffLines.length) {
    const diffEl = row.createDiv({ cls: 'yapi-write-edit-diff' });
  ...
  row.createDiv({
    cls: toolCall.status === 'running' ? 'yapi-write-edit-loading' : 'yapi-write-edit-done-text',
