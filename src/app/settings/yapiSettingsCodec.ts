...
import type {
  YapiSettingsCodec,
  YapiSettingsNormalizationResult,
} from "@yapi/obsidian-host/settings/yapiSettingsStorage";
...
import { DEFAULT_AGENT_SETTINGS, DEFAULT_YAPI_SETTINGS } from "@yapi/yapi-agent-core/foundation/settingsDefaults";
...
function normalizeChatViewPlacement(value: unknown): ChatViewPlacement {
  ...
  return DEFAULT_YAPI_SETTINGS.chatViewPlacement;
}
...
export function normalizeStoredYapiSettings(
  stored: Record<string, unknown>,
): YapiSettingsNormalizationResult {
  ...
  const autoCompactThresholdRatio = clampNumber(
    ...
    DEFAULT_YAPI_SETTINGS.autoCompactThresholdRatio,
    0.5,
    0.95,
  );
  const autoCompactKeepRecentTokens = Math.round(clampNumber(
    stored.autoCompactKeepRecentTokens,
    DEFAULT_YAPI_SETTINGS.autoCompactKeepRecentTokens,
    1_000,
    200_000,
  ));
  ...
  const settings: YapiSettings = {
    ...
    enableAutoCompact: typeof stored.enableAutoCompact === "boolean"
      ...
      : DEFAULT_YAPI_SETTINGS.enableAutoCompact,
...
function stripDeviceLocalSettings(settings: YapiSettings): YapiSettings {
...
export function createYapiSettingsCodec(
  ...
  return {
    getDefaults() {
      const settings = {
        ...DEFAULT_YAPI_SETTINGS,
        agentSettings: { ...DEFAULT_YAPI_SETTINGS.agentSettings },
      };
    ...
    normalize(stored) {
      const result = normalizeStoredYapiSettings(stored);
      if (!deviceLocalExternalContexts) {
        return result;
      }
