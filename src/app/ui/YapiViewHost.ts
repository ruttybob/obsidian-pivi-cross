...
const logger = new PluginLogger('YapiViewHost');
...
export class YapiViewHost extends ItemView {
  ...
  constructor(
    ...
    // overwritten by prototype patching. Hover Editor patches YapiViewHost.prototype.load
  ...
  async onOpen(): Promise<void> {
    ...
    if (!ownerWindow) {
      throw new Error('Yapi chat view has no owning window.');
    }
    container.empty();
    container.createDiv({ cls: 'yapi-loading', text: appI18n.t('common.loading') });
    ...
    try {
      ...
      try {
        ...
        throw new AggregateError(
          ...
          'Yapi chat mount and cleanup both failed.',
        );
      }
      throw mountError;
    }
  ...
  private async disposeChatRuntimeSurface(): Promise<void> {
    ...
    if (errors.length > 1) {
      throw new AggregateError(errors, 'Yapi chat persistence and disposal both failed.');
    }
  }
