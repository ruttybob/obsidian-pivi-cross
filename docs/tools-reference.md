# Yapi tools reference

Yapi registers the following tools with the Pi agent when their prerequisites are available. Obsidian tool rows can be enabled or disabled from **Settings → Yapi → Tools**; Web Search, MCP, Skills, and Subagents use their own settings sections. Disabled or unavailable tools are omitted from subsequent turns.
...
`obsidian_history` and `obsidian_tasks` register only when Yapi's CLI integration is enabled and the official Obsidian CLI is available.
...
External access is disabled by default and requires `allowExternalRead` plus at least one allowed root. Roots come from the vault-scoped device-local overlay or folders attached for the current turn; absolute paths are not written to synced `.yapi/settings.json` or session JSONL. `obsidian_command` and `obsidian_eval` additionally require the official Obsidian CLI. `obsidian_bash` does not require the CLI, but accepts only allowlisted one-line commands and rejects shell control syntax.
...
Web tools are configured under **Settings → Yapi → Web Search**. Their runtime availability depends on the selected providers and credentials.

## Extensions

| Tool | Purpose | Mutates |
|------|---------|---------|
| `mcp` | Invoke vault-local MCP servers from `.yapi/mcp.json` | Varies |
| `skill` | Load vault-local skill instructions from `.yapi/skills/` | No |
