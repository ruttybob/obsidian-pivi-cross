...
- Deterministic invariants exist only in Jest: `tests/yapi-react/chatUiStore.test.tsx` (hundreds of one-entity updates coalesce into one animation-frame commit; latest-100 projection of a 5K session; entity publishes do not notify unrelated messages) and `tests/yapi-react/MessageList.test.tsx` (5K transcript keeps mounted row count bounded by viewport + overscan).
...
- The projection publish path is `ChatProjectionStore.queueUpsert()` in `packages/yapi-react/src/store/chatProjectionStore.ts` (coalesced per `ownerWindow.requestAnimationFrame`); Markdown renders through imperative adapters mounted by `src/app/ui/imperativeChatMessagePresentation.ts`.
...
- [x] A fixture generator script can create test sessions in a vault's `.yapi/sessions/`: 1K messages, 5K messages, one 100KB Markdown message, 20 Agent runs. Verified in the configured vault with the real Pi `SessionManager`, followed by an Obsidian reload with no captured errors.
- [x] The measurement protocol (scenarios, environment fields to record: Obsidian/Yapi version, window type main/pop-out, scenario shape) is written down in docs, and one baseline run is recorded before specs 002-004 change behavior. Scenarios include 1K/5K cold open, older-page load, 100KB Markdown streaming, 20 Agent runs, scrolling away from the end, late background events, repeated prepend, and session switching.
...
- Trace instrumentation seams inside `packages/yapi-react/src/store/chatProjectionStore.ts`, `packages/yapi-react/src/chat/messages/MessageList.tsx`, and the Markdown adapter mount path in `src/app/ui/imperativeChatMessagePresentation.ts` (hook points only, behavior unchanged when disabled).
...
|---|---|---|---|
| 2026-07-15 | Instrumentation hooks live behind one injected recorder interface, default no-op | Keeps `packages/yapi-react` free of host/debug imports and satisfies the no-production-logging rule | WS-01, WS-02 |
...
| 2026-07-15 | Trace JSON is written under vault-local `.yapi/perf-traces/` with environment and scenario metadata; no trace data enters settings or sessions | Keeps exported evidence next to the measured vault without changing durable product state | WS-02, WS-04 |
...
| 2026-07-15 | The start command reads an optional one-line `.yapi/perf-scenario.txt`, defaulting to `manual` | Obsidian CLI cannot execute a command blocked on `window.prompt`; a vault-local dev input keeps scenario runs scriptable without adding settings or session state | WS-02, WS-04 |
...
|---|---|---|---|---|---|
| WS-01 | `ChatPerfRecorder` interface + no-op default, hook points in projection store commit/flush/paint, MessageList mount/measure/anchor drift, Markdown adapter mount/update | Codex | Done | None | `npm run test -- tests/yapi-react`, `npm run check:boundaries` |
...
3. Do not import Obsidian, Node, or app modules inside `packages/yapi-react` (architecture check will fail otherwise).

## Verification

- `npm run typecheck && npm run lint && npm run check:boundaries && npm run test:coverage && npm run build && npm run check:bundle-size`
- Manual scenario runs per docs/11 Performance observability list, recorded with environment metadata (Obsidian version, Yapi commit, window type, scenario name).
...
- Nearest local guidance: `packages/yapi-react/AGENTS.md` (recorder seam), `scripts/AGENTS.md` (new script).
...
- Evidence: `npm run typecheck`; `npm run lint`; `npm run check:boundaries`; `npm run test -- --runInBand tests/yapi-react tests/unit/app/ui/createStreamingMarkdownContentAdapter.test.ts` (24 suites, 169 tests); `npm run build`; `obsidian reload`; `obsidian dev:errors` (`No errors captured.`).
...
- Evidence: `npm run typecheck`; `npm run lint`; `npm run check:boundaries`; full `npm run test -- --runInBand` (228 suites, 1,683 tests); production `npm run build` plus negative grep for `yapi-chat-perf-v1` and all debug command IDs; `npm run check:bundle-size` (2.84 MB); development bundle registered all three commands in Obsidian; real-vault smoke trace `2026-07-15T09-25-14-928Z-manual.json` exported with schema `yapi-chat-perf-v1`, Obsidian 1.13.2 / Yapi 0.9.0 metadata, main-window long-task support, three heap samples, and a virtual-row sample; production bundle restored and `obsidian dev:errors` reported `No errors captured.`
- Remaining: WS-03 through WS-05.
- Blockers: none. CLI cannot create hidden `.yapi` files directly in this environment, so the smoke run used the documented `manual` fallback; named baseline runs may write the one-line scenario file through the vault adapter or filesystem.
...
- Evidence: nine named `yapi-chat-perf-v1` traces cover main-window and pop-out cold open, older paging, repeated prepend, 20 Agent runs, scroll-away/late update, 100KB streaming, and isolated switching. The final switching trace recorded max 25 rows / 758 DOM nodes and left `.yapi/tab-manager-state.json` byte-identical; production was rebuilt/deployed and `obsidian dev:errors` returned `No errors captured.`
