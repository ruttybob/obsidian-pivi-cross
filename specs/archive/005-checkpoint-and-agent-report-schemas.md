...
- Subagent trace persistence is split between `YAPI_MESSAGE_UI` entries (`YapiMessageUiData.toolCalls[].subagent`) and the subagent's own JSONL (`packages/yapi-agent-core/src/session/subagentJsonl.ts`, `extractFinalResultFromSubagentJsonl`).
...
- Schema types in `packages/yapi-agent-core/src/session/types.ts` (or a sibling module) plus tolerant parse/serialize helpers.
...
| 2026-07-16 | Store checkpoints in Pi's existing `compaction.details.yapiCheckpoint` extension slot | Pi 0.80.6 exposes typed compaction details; the plain summary, leaf identity, append-only bytes, fork behavior, and unmodified Pi context semantics remain unchanged | WS-01, WS-03, WS-05 |
...
1. Read `packages/yapi-agent-core/src/engine/pi/AGENTS.md` before editing; raw Pi SDK types stay inside `engine/pi`.
...
- Nearest local guidance: `packages/yapi-agent-core/src/engine/pi/AGENTS.md`, `packages/yapi-agent-core/AGENTS.md`.
...
- Problems recorded: no provenance-verifiable Yapi 0.7.0 JSONL bytes exist in the repository. The success criterion's “0.7.0-era” label is satisfied only as a documented legacy v1 shape contract; the fixture README prohibits presenting it as a captured historical session.
...
- Changed: built/deployed production, reloaded Yapi, ran isolated non-UI synthetic sessions through real Pi runtime paths, and deleted every temporary session afterward. No existing tab or window was created, switched, or modified.
...
- Cleanup: zero synthetic session markers, one pre-existing Yapi leaf, zero floating windows, zero captured Obsidian errors, and a clean worktree before closeout.
- Remaining: none.
- Blockers: none.
