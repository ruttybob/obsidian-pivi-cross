...
| Package contracts | `packages/*/AGENTS.md`, `packages/yapi-agent-core/src/engine/pi/AGENTS.md` | Package entrypoints, dependency boundaries, or gotchas change |
| Feature maps | `src/ui/AGENTS.md`, `src/ui/chat/AGENTS.md`, `src/ui/chat/rendering/AGENTS.md`, `src/ui/shared/AGENTS.md`, `src/ui/inline-edit/AGENTS.md`, `src/app/AGENTS.md`, `packages/yapi-react/AGENTS.md`, `packages/yapi-react/src/i18n/AGENTS.md`, `packages/yapi-react/styles/AGENTS.md`, `src/ui/chat/ui/file-context/AGENTS.md`, `src/ui/chat/ui/file-context/state/AGENTS.md`, `src/ui/chat/ui/file-context/view/AGENTS.md` | Local UI/runtime flow or seam rules change |
...
For UI or runtime changes that the user needs to inspect inside Obsidian, run the project build and reload the plugin before handing back control. The normal path is `npm run build` followed by `obsidian plugin:reload id=yapi`, unless the user explicitly asks not to reload or the Obsidian CLI is unavailable.
...
| User-visible UI text | Always `packages/yapi-react/src/i18n/` in the **same commit** (see Coding Standards) |

---

## 🤖 Agent skills

This repo does not track repo-local agent skills. Keep developer narrative in `docs/` and operational project guidance in this file and package/local `AGENTS.md` files; runtime vault skills live under each vault's `.yapi/skills/` directory.

**Vault default bundle** (end users, not this repo): first vault load may prompt to install [kepano/obsidian-skills](https://github.com/kepano/obsidian-skills) into `<vault>/.yapi/skills/`, but installation/updating must happen only after explicit user confirmation.
...
Nested `AGENTS.md` files under `src/`, `tests/`, `packages/`, and `packages/yapi-agent-core/src/engine/pi/` are directory/package maps (`init-deep` or hand-maintained); treat root `AGENTS.md` as authoritative for cross-cutting rules. The hierarchy is:

- **Root** `AGENTS.md` — repo-wide build, test, release, seam rules, glossary, and quality gates.
- **Package** `packages/*/AGENTS.md` — package purpose, entrypoints, boundaries, verification. `packages/yapi-agent-core/src/engine/pi/AGENTS.md` covers the Pi engine adapter boundary.
...
- **React UI** `packages/yapi-react/AGENTS.md` — presentation boundary, locale catalogs, styles, and React ownership rules.
- **i18n** `packages/yapi-react/src/i18n/AGENTS.md` — translator API, locale catalogs, translation commit policy.
- **Styles** `packages/yapi-react/styles/AGENTS.md` — CSS architecture, manifest order, build flow, conventions.
...
| **Tab binding** | UI tab's durable binding to `sessionFile` plus draft UI state such as selected model. | `.yapi/tab-manager-state.json` and tab restore logic; `data.json.tabManagerState` is legacy migration input only. | Deprecated chat-id fields or `leafId` as durable tab identity. |
...
| **Checkpoint** | Versioned structured continuation state stored additively in `compaction.details.yapiCheckpoint` while retaining the readable Pi summary. | Compaction persistence, chained decision/artifact ledger, future checkpoint presentation. | Replacing Pi compaction fields or treating malformed details as required context. |
...
flowchart TD
  ...
  AppUI --> ReactUI["@yapi/yapi-react<br/>React chrome + i18n/styles"]
  AppUI --> RuntimeUI
  App --> HostPkg["@yapi/obsidian-host"]
  App --> ToolsPkg["@yapi/obsidian-tools"]
  App --> CorePkg["@yapi/yapi-agent-core"]
...
Allowed edges at this layer: composition (`src/app`) may depend on every package and on `src/ui`. Presentation (`@yapi/yapi-react`) may depend on non-engine core contracts/models. Product adapters (`src/ui`) may depend on non-engine core APIs, the three approved React presentation subpaths, public Obsidian APIs, narrowly scoped Node `fs`/`os`/`path` modules used by imperative adapters, `@/app/i18n`, `@/app/hostPlatform`, and type-only host contracts. Host and tools never import UI or `engine/pi`. `engine/pi` never imports `@yapi/obsidian-host` and its `PiRuntimeHost` has no workspace peek. Only `src/app/ui` may call chat/settings package mount APIs or implement their feature ports; package-owned inline-edit widgets mount their own React surface internally. `src/ui` must not import `src/app/ui` or call `getPiWorkspace()`, `getUiFacades()`, `saveSettings()`, or `getAllViews()` (enforced by architecture checks). Chat ports take an explicit workspace argument from composition; `YapiChatHost` remains an `app`-only runtime host, while `YapiChatCompositionHost` carries composition-only capabilities.
...
flowchart TD
  ...
  AppUI --> ReactUI["@yapi/yapi-react<br/>ChatShell / SettingsRoot"]
  ...
  Engine -. "runtime persistence through injected ports" .-> Vault["Vault .yapi/*"]
```
...
flowchart LR
  ...
  Store --> Shell["@yapi/yapi-react ChatShell"]
  Shell --> Adapters["adapter slots in src/ui"]
  Engine --> Session[".yapi/sessions"]
```

Chat chrome and settings live in `@yapi/yapi-react`. The app-owned imperative adapter closure captures core-owned `ChatPorts` for `TabManager`; the React mount contract never sees them. `ChatShell` consumes snapshots/actions, while `SettingsRoot` consumes React-owned `SettingsPorts` implemented in `src/app/ui`. Chrome/usage state reaches React through `ChatUiStore`; virtual message order/entities reach it through `ChatProjectionStore`; `ActiveChatUiBridge` selects both. `src/ui/chat` owns runtime orchestration and imperative content adapters only.
...
flowchart TD
  ...
  AppUI --> ReactUI["@yapi/yapi-react"]
  AppUI --> ChatUI["src/ui"]
  App --> Host["@yapi/obsidian-host"]
  App --> Tools["@yapi/obsidian-tools"]
  App --> Engine["core/engine/pi"]
  App --> Core["@yapi/yapi-agent-core<br/>non-engine APIs"]
  ReactUI --> Core
  ChatUI --> Core
  Tools --> Host
  Tools --> Core
...
- Keep `legacy-peer-deps=true` until `npm install --dry-run --ignore-scripts --legacy-peer-deps=false` succeeds. As of 2026-07-11, `obsidian@1.13.1` requires exact `@codemirror/state@6.5.0` while YaPi uses `^6.7.1`; `eslint-plugin-obsidianmd` also brings ESLint-9-only peers under an ESLint 10 root.
...
```bash
npm run build && obsidian plugin:reload id=yapi
```
...
**Obsidian plugin folder layout:** Deploy only `main.js`, `manifest.json`, and `styles.css`. Obsidian may also create `data.json` at runtime. Do not copy CLI entrypoints, `node_modules`, or other pi-coding-agent artifacts into `.obsidian/plugins/yapi/` — the esbuild `copy-to-obsidian` plugin prunes stale files on each build.
...
Enable `yapi` after the first deployment; this step is not needed for subsequent builds:
```bash
obsidian plugin:enable id=yapi
```

#### Step D: Reload the plugin
Reload only `yapi` after subsequent builds so the rest of the vault remains undisturbed:
```bash
...
```bash
obsidian command id=yapi:open-view
```
...
1. **Pi-only Service Boundary**: Feature/app code uses YaPi-owned package APIs (`@yapi/*`) and the app shell. Avoid importing low-level external Pi SDK packages or MCP SDKs outside the Pi engine/tooling layer.
2. **Ports & DI for host capabilities**: `packages/yapi-agent-core` (including `engine/pi`) depends on `ports/` contracts, not `@yapi/obsidian-host`. App composition injects host adapters (files, secrets, HTTP, process).
3. **UI over service contracts**: `src/ui/**` may use `PiChatService` / `AuxQueryRunner` from `@yapi/yapi-agent-core/runtime`, injected core-owned `ChatPorts`, and the `app`-only `YapiChatHost`. Chat runtime/session/model/catalog/settings behavior must use `ChatPorts`; `YapiChatCompositionHost` and `YapiSettingsHost` stay in app composition. UI must not import `@yapi/yapi-agent-core/engine/pi/**`, `src/app/workspace/**`, `@/app/ui/**`, or `@yapi/obsidian-host/**` (use `@/app/hostPlatform` instead), and must never call `getPiWorkspace()`, `getUiFacades()`, `saveSettings()`, or `getAllViews()`.
4. **One-way app → UI composition**: `src/app/workspace/**` must not import `@/ui/**`. Host contracts must not import concrete `YapiViewHost`, app workspace implementation modules, or `engine/pi` types. Composition root (`serviceGraph`, registrations, `src/app/ui`) may import UI modules to mount React roots and wire adapters. Do not inject a settings renderer into the service graph—settings mount only via `YapiSettingTabHost` + `SettingsPorts`.
...
7. **Pi Dependency Boundary**: `packages/yapi-agent-core/src/engine/pi/` is the Pi SDK boundary. UI, tools, host, MCP, and skills packages depend on YaPi-owned contracts, not raw Pi SDK packages.
...
10. **UI text requires i18n (every commit)**: Any change that adds or edits **user-visible** UI copy (settings labels/descriptions, buttons, Notices, placeholders, aria-labels, command/ribbon names, chat chrome, empty states, tool display labels, modals, etc.) **must** ship i18n in the **same commit**:
   - Add/update keys in `packages/yapi-react/src/i18n/locales/en.json` (canonical), then mirror the key tree in **all** other locale JSON files with translations.
   ...
   - Packages other than `@yapi/yapi-react` receive translated strings when host/package code surfaces Notices or labels; they must not import app translator state.
   ...
   - Details and catalog workflow: `packages/yapi-react/src/i18n/AGENTS.md`.
...
- Do not keep UI-named facade files that only re-export package-layer helpers. Import those helpers from the owning `@yapi/*` package instead, or delete the unused facade.
...
- **Standard release path (preferred):** use Conventional Commits on `main`, let Release Please open the release PR, review/merge that PR, and let `.github/workflows/release-please.yaml` create the GitHub Release and upload `main.js`, `manifest.json`, and `styles.css`. While YaPi is pre-1.0, `fix` commits produce patch releases and `feat` commits produce minor releases. README version badge updates come from `node scripts/sync-version.js`; do not add release-please README markers.
...
YaPi-native agent tools (`packages/obsidian-tools/`) prefer the **in-process Obsidian Plugin API**. The official CLI is used only where the public API cannot satisfy the operation: history, tasks, daily notes, Base queries, and the separately gated `command` / `eval` tools.
